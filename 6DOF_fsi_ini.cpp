...
void sixdof_fsi::ini(lexer *p, fdm *a, ghostcell *pgc, vector<net*>& pnet)
{
	...
	else
	{
		...
		Xms.resize(p->mooring_count);
		Yms.resize(p->mooring_count);
		Zms.resize(p->mooring_count);
		Kms.resize(p->mooring_count);
		Mms.resize(p->mooring_count);
		Nms.resize(p->mooring_count);
...
void sixdof_fsi::ini_parameter(lexer *p, fdm *a, ghostcell *pgc)
{
    // Prescribed motions
...
void sixdof_fsi::iniPosition_RBM(lexer *p, fdm *a, ghostcell *pgc)
{
...
void sixdof_fsi::ini_parallel(lexer *p, fdm *a, ghostcell *pgc)
{
