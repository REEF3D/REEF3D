...
void sixdof_fsi::predictor(lexer *p, fdm *a, ghostcell *pgc, double alpha)
{
}
    

bool sixdof_fsi::corrector(lexer *p, fdm *a, ghostcell *pgc, double alpha, vrans *pvrans, vector<net*>& pnet)
{
    double err_norm = 0.0;
...
void sixdof_fsi::prescribedMotion(lexer *p, fdm *a, ghostcell *pgc, Eigen::Vector3d& dp, Eigen::Vector3d& dc)
{
    if (p->X11_u == 2)
    {
        dp(0) = 0.0; 
