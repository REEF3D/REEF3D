...
void bicgstab::setup(lexer* p,fdm* a, ghostcell* pgc, int var, cpt &C)
{
}

void bicgstab::start(lexer* p,fdm* a, ghostcell* pgc, field &f, vec& xvec, vec& rhsvec, int var, int gcv, double stop_crit)
{
	p->preconiter=0;
...
void bicgstab::solve(lexer* p,fdm* a, ghostcell* pgc, vec& xvec, vec& rhsvec, int var, int gcv, int &solveriter, int maxiter, double stop_crit, cpt &C)
{
	solveriter=0;
	residual = 1.0e9;
...
void bicgstab::matvec_axb(lexer *p,fdm* a, vec &x, vec &y, cpt &C)
{
	NLOOP
	{
	y.V[n]  = a->rhsvec.V[n]
...
void bicgstab::matvec_std(lexer *p,fdm* a, vec &x, vec &y, cpt &C)
{
	NLOOP
	{
...
double bicgstab::res_calc(lexer *p, fdm *a, vec &x, ghostcell *pgc, cpt &C)
{
	double y;
	double resi=0.0;
...
void bicgstab::gcpara_update(lexer* p, vec &x, ghostcell* pgc)
{
}

void bicgstab::gcupdate(lexer *p, fdm *a, ghostcell *pgc, vec &x, int var, int gcv, int solveriter)
{
...
void bicgstab::fillxvec1(lexer* p, fdm* a, field& f)
{
	int count,q;
...
void bicgstab::fillxvec3(lexer* p, fdm* a, field& f)
{
	int count,q;
