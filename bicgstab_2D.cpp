...
bicgstab_2D::bicgstab_2D(lexer* p, fdm *a, ghostcell *pgc, int precon_select):sj(p),rj(p),r0(p),vj(p),tj(p),pj(p),precoeff(p),
												ph(p),sh(p),epsi(1e-19)
{
	precon123 = new jacobi_scaling(p,a,pgc);
	
	if(precon_select==0)
	precon4 = new solver_void(p,a,pgc);
	
	if(precon_select==1)
	precon4 = new jacobi_scaling(p,a,pgc);
	
	if(precon_select==2)
	precon4 = new jacobi_block(p,a,pgc);
	
	if(precon_select==3)
	precon4 = new sip(p,a,pgc);
    ...
    {
    cout<<endl;
    cout<<"!!! Invalid Solver-Preconditioner selection, please check the manual!"<<endl;
    cout<<"!!! Use HYPRE BiCGSTAB + PFMG for best performance."<<endl<<endl<<endl<<endl;
    }
    MPI_Finalize();
    exit(0);
    }
...
bicgstab_2D::~bicgstab_2D()
{
}

void bicgstab_2D::setup(lexer* p,fdm* a, ghostcell* pgc, int var, cpt &C)
{
}

void bicgstab_2D::start(lexer* p,fdm* a, ghostcell* pgc, field &f, vec& xvec, vec& rhsvec, int var, int gcv, double stop_crit)
{
	p->preconiter=0;
...
void bicgstab_2D::startF(lexer* p, fdm_fnpf* c, ghostcell* pgc, double *f, vec& rhsvec, matrix_diag &M, int var, int gcv, double stop_crit)
{
}
	
void bicgstab_2D::solve(lexer* p,fdm* a, ghostcell* pgc, vec& xvec, vec& rhsvec, int var, int gcv, int &solveriter, int maxiter, double stop_crit, cpt &C)
{
	solveriter=0;
	residual = 1.0e9;
...
void bicgstab_2D::matvec_axb(lexer *p,fdm* a, vec &x, vec &y, cpt &C)
{
	NLOOP
	{
	y.V[n]  = a->rhsvec.V[n]
...
void bicgstab_2D::matvec_std(lexer *p,fdm* a, vec &x, vec &y, cpt &C)
{
	NLOOP
	{
...
double bicgstab_2D::res_calc(lexer *p, fdm *a, vec &x, ghostcell *pgc, cpt &C)
{
	double y;
	double resi=0.0;
...
void bicgstab_2D::gcpara_update(lexer* p, vec &x, ghostcell* pgc)
{
}

void bicgstab_2D::gcupdate(lexer *p, fdm *a, ghostcell *pgc, vec &x, int var, int gcv, int solveriter)
{
...
void bicgstab_2D::fillxvec1(lexer* p, fdm* a, field& f)
{
	int count,q;
...
void bicgstab_2D::fillxvec2(lexer* p, fdm* a, field& f)
{
	int count,q;
...
void bicgstab_2D::fillxvec3(lexer* p, fdm* a, field& f)
{
	int count,q;
...
void bicgstab_2D::fillxvec4(lexer* p, fdm* a, field& f)
{
	int count,q;
...
void bicgstab_2D::finalize(lexer *p, fdm *a, field &f, vec &xvec, int var)
{
	if(var==1)
    {
        count=0;
