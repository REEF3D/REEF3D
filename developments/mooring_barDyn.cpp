...
mooring_barDyn::mooring_barDyn(int number):line(number){}

mooring_barDyn::~mooring_barDyn(){}

void mooring_barDyn::start(lexer *p, fdm *a, ghostcell *pgc)
{
	...
	double phi = 0.0;
	t_net_n = t_net;
	t_net = phi*p->simtime + (1.0 - phi)*(p->simtime + p->dt);
	double dtm = t_net - t_net_n;
	
    dt_ = dtm; //p->X325_dt > 0.0 ? min(dtm, p->X325_dt) : dtm;
    ...
    if (p->mpirank == 0)
    {
        cout<<"Line converged after "<<convIt.transpose()<<" iterations"<<endl;
        cout<<"Mooring time: "<<endtime1<<endl;    
    }
}


void mooring_barDyn::startLoop
(
	lexer *p, 
	fdm *a, 
	ghostcell *pgc,
    ...
    else
    {
        ...
        for (int it = 1; it < 20; it++)
        {
	        ...
	        { 
	           ...
	           /* if (p->mpirank == 0)
		        {
		            cout<<"Net converged after "<<it<<" iterations"<<endl;
		        }*/
		        break;
            }
        }
...
void mooring_barDyn::limitTension()
{ 
...
void mooring_barDyn::updateAcc(lexer *p, fdm *a, ghostcell *pgc)
{
...
void mooring_barDyn::updateTopAcc(lexer *p)
{
...
void mooring_barDyn::updateField(lexer *p, fdm *a, ghostcell *pgc, int cmp)
{
	int *recField, *count;
...
Eigen::VectorXd mooring_barDyn::timeWeight(lexer* p)
{	
