...
void fnpf_sg_laplace_cds4::start(lexer* p, fdm_fnpf *c, ghostcell *pgc, solver *psolv, fnpf_sg_fsfbc *pf, double *f)
{
	...
	FLOOP
	{
        if(p->flag7[FIJK]>0)
        {
            ...
            c->rhsvec.V[n] += sigxyz2*( f[FIJKp3]*(1.0/Z4))*p->z_dir;

            
            c->M.n[n] += abb*4.0*p->DZN[KP]*c->Bx(i,j)/(denom*(p->DXP[IP] + p->DXP[IM1]));
            c->M.s[n] += -abb*4.0*p->DZN[KP]*c->Bx(i,j)/(denom*(p->DXP[IP] + p->DXP[IM1]));
            c->M.e[n] += abb*4.0*p->DZN[KP]*c->By(i,j)/(denom*(p->DYP[JP] + p->DYP[JM1]));
            c->M.w[n] += -abb*4.0*p->DZN[KP]*c->By(i,j)/(denom*(p->DYP[JP] + p->DYP[JM1]));
            c->M.tt[n] += abb;
            c->M.bb[n] = 0.0;
            }
        }
