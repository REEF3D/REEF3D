...
void fnpf_sg_laplace_cds4_v2::start(lexer* p, fdm_fnpf *c, ghostcell *pgc, solver *psolv, fnpf_sg_fsfbc *pf, double *f)
{
    ...
    LOOP
    {
        ...
        X0 = -p->XP[IP2] + 27.0*p->XP[IP1] - 27.0*p->XP[IM1] + p->XP[IM2];
        ...
        Y0 = -p->YP[JP2] + 27.0*p->YP[JP1] -27.0*p->YP[JP] + p->YP[JM1];
        ...
        Z0 = -p->ZP[KP2] + 27.0*p->ZP[KP1] -27.0*p->ZP[KP] + p->ZP[KM1];        
	...
	LOOP
	{
        if(p->flag7[FIJK]>0)
        {
            ...
            if(p->flag7[FIJKp4]<0)
            {
            c->rhsvec.V[n] -= c->M.ttt[n]*f[FIJKp4];
            c->M.ttt[n] = 0.0;
            }
