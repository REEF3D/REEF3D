...
void ghostcell::forcing2(lexer *p, fdm *a, field& f, field &uvel, field &vvel, field &wvel, double alpha)
{
        ...
        || ((p->gcb2[n][4]==21 || p->gcb2[n][4]==22) && p->B19==1))
	{
		...
		L	 +=	  sqd*((f(i+1,j,k)-b_ijk)*visc_ddx_p
			...
			+ 2.0*sqd*((f(i,j+1,k)-b_ijk)*(a->visc(i,j+1,k)+a->eddyv(i,j+1,k))
				 -(b_ijk-f(i,j-1,k))*(visc_ijk+ev_ijk))
        ...
        // Pressure
        L -= PORVAL2*(a->press(i,j+1,k)-a->press(i,j,k))/(p->DXM*pdens->roface(p,a,0,1,0));
