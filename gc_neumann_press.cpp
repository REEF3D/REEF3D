...
void ghostcell::neumann_press(lexer *p,field& f,double dist,int gcv, int bc, int cs)
{
	...
	ys=1;
    if(dist>0.5*deltax*(1.0-1.0e-6) && dist<0.5*deltax*(1.0+1.0e-6))
    ys=0;
...
//fill pos[]
	...
	pos[m]=-deltax*double(orderpress-m-2)-0.5*deltax;

	pos[orderpress-2]=-0.5*deltax;
	pos[orderpress-1]=dist;

	for(m=0;m<margin;m++)
	x[m] = dx*double(m+2-ys)-0.5*deltax;

//fill y[]
	if(cs==1 )
	for(m=0;m<orderpress-1;m++)
	y[m]=(f(i+orderpress-m-1,j,k)-f(i+orderpress-m-2,j,k))/deltax;

	if(cs==2)
	for(m=0;m<orderpress-1;m++)
	y[m]=(f(i,j-orderpress+m+2,k)-f(i,j-orderpress+m+1,k))/deltax;

	if(cs==3)
	for(m=0;m<orderpress-1;m++)
	y[m]=(f(i,j+orderpress-m-1,k)-f(i,j+orderpress-m-2,k))/deltax;

	if(cs==4)
	for(m=0;m<orderpress-1;m++)
	y[m]=(f(i-orderpress+m+2,j,k)-f(i-orderpress+m+1,j,k))/deltax;

	if(cs==5)
	for(m=0;m<orderpress-1;m++)
	y[m]=(f(i,j,k+orderpress-m-1)-f(i,j,k+orderpress-m-2))/deltax;

	if(cs==6)
	for(m=0;m<orderpress-1;m++)
	y[m]=(f(i,j,k-orderpress+m+2)-f(i,j,k-orderpress+m+1))/deltax;
	...
	cout<<f(i,j,k)<<" . ";
	for(q=0;q<margin;++q)
	cout<<f(i,j,k) - double(q+1)*deltax*y[orderpress+q]<<" ";
...
// write extrapolated ghost cell values into f()
	...
	if(cs==1)
	for(q=0;q<margin;++q)
	f(i-q-1,j,k)=f(i,j,k) - (double(q)*deltax + deltax)*y[orderpress+q-1+ys];

	if(cs==2)
	for(q=0;q<margin;++q)
	f(i,j+q+1,k)=f(i,j,k) + (double(q)*deltax + deltax)*y[orderpress+q-1+ys];
	...
	if(cs==5)
	for(q=0;q<margin;++q)
	f(i,j,k-q-1)=f(i,j,k) - (double(q)*deltax + deltax)*y[orderpress+q-1+ys];
	...
	if(cs==5)
	for(q=0;q<margin;++q)
	f(i,j,k-q-1)=f(i,j,k) - double(q+1)*deltax*wallvalue;
