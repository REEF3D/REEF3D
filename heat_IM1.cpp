...
heat_IM1::heat_IM1(lexer* p, fdm* a, ghostcell *pgc,heat *&pheat) : ibcheat(p), heat_print(p,a), Tn(p), thermdiff(p)
{
	gcval_heat=80;
}
...
void heat_IM1::start(fdm* a, lexer* p, convection* pconvec, diffusion* pdiff, solver* psolv, ghostcell* pgc, ioflow* pflow)
{
    starttime=pgc->timer();
    diff_update(p,a,pgc);
    clearrhs(p,a,pgc);
...
void heat_IM1::timesource(lexer* p, fdm* a, field& fn)
{
    int count=0;
    int q;
    ...
    LOOP
    {
        ...
        a->rhsvec.V[count] += a->L(i,j,k) + Tn(i,j,k)/PDT;

	++count;
    }
}

void heat_IM1::ttimesave(lexer* p, fdm* a)
{
    LOOP
    Tn(i,j,k)=T(i,j,k);
}

void heat_IM1::diff_update(lexer *p, fdm *a, ghostcell *pgc)
{
...
void heat_IM1::clearrhs(lexer *p, fdm *a, ghostcell *pgc)
{
    int n=0;
	LOOP
	{
