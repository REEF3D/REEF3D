...
void idiff2_FS::diff_v(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &u, field &v, field &w, double alpha)
{
    ...
    VLOOP
    {
	visc_ijk=a->visctot(i,j,k);
	visc_im_j_k=a->visctot(i-1,j,k);
	visc_ip_j_k=a->visctot(i+1,j,k);
	visc_i_jp_k=a->visctot(i,j+1,k);
	visc_i_j_km=a->visctot(i,j,k-1);
	visc_i_j_kp=a->visctot(i,j,k+1);
	
	visc_ddx_p = 0.25*(visc_ijk + visc_i_jp_k + visc_ip_j_k + a->visctot(i+1,j+1,k));
    
	visc_ddx_m = 0.25*(visc_im_j_k + a->visctot(i-1,j+1,k) + visc_ijk + visc_i_jp_k);
    
	visc_ddz_p = 0.25*(visc_ijk + visc_i_jp_k + visc_i_j_kp + a->visctot(i,j+1,k+1));

	visc_ddz_m = 0.25*(visc_i_j_km + a->visctot(i,j+1,k-1) + visc_ijk + visc_i_jp_k);
    
	
	a->M.p[count] = 2.0*(visc_i_jp_k)/(p->DYN[JP]*p->DYP[JP])
				  + 2.0*(visc_ijk+ev_ijk)/(p->DYN[JM1]*p->DYP[JP])
	...
	a->rhsvec.V[count] += ((u(i,j+1,k)-u(i,j,k))*visc_ddx_p - (u(i-1,j+1,k)-u(i-1,j,k))*visc_ddx_m)/(p->DYP[JP]*p->DXN[IP])
	 ...
	 a->M.e[count] = -2.0*(visc_ijk)/(p->DYN[JM1]*p->DYP[JP]);
	 a->M.w[count] = -2.0*(visc_i_jp_k)/(p->DYN[JP]*p->DYP[JP]);
