...
kepsilon_RK2::kepsilon_RK2(lexer* p, fdm* a, ghostcell *pgc) : kepsilon(p,a,pgc)
{
	gcval_kin=20;
	gcval_eps=30;
...
kepsilon_RK2::~kepsilon_RK2()
{
}

void kepsilon_RK2::start(fdm* a, lexer* p, convection* pconvec, diffusion* pdiff,solver* psolv, ghostcell* pgc, ioflow* pflow)
{
    field4 ark1(p);
    field4 brk1(p);
	Pk_update(p,a,pgc);
...
// Step 2
	...
	pdiff->diff_scalar(p,a,pgc,psolv,ark1,a->visc,ke_sigma_k,0.5);

	LOOP
	kin(i,j,k) = 0.5*kin(i,j,k) + 0.5*ark1(i,j,k)
				+ 0.5*p->turbtimestep*a->L(i,j,k);
	
	pdiff->idiff_scalar(p,a,pgc,psolv,kin,a->visc,ke_sigma_k,0.5);
	...
	pdiff->diff_scalar(p,a,pgc,psolv,brk1,a->visc,ke_sigma_e,0.5);

	LOOP
	eps(i,j,k) = 0.5*eps(i,j,k) + 0.5*brk1(i,j,k)
				+ 0.5*p->turbtimestep*a->L(i,j,k);

	pdiff->idiff_scalar(p,a,pgc,psolv,eps,a->visc,ke_sigma_e,0.5);
...
void kepsilon_RK2::ktimesave(lexer *p, fdm* a, ghostcell *pgc)
{
}

void kepsilon_RK2::etimesave(lexer *p, fdm* a, ghostcell *pgc)
{
}
