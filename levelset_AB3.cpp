...
levelset_AB3::levelset_AB3(lexer* p, fdm *a, ghostcell* pgc, heat *&pheat, concentration *&pconc):lab1(p),lab2(p)
{
	if(p->F50==1)
	gcval_phi=51;
...
void levelset_AB3::start(fdm* a,lexer* p, convection* pconvec,solver* psolv, ghostcell* pgc,ioflow* pflow, reini* preini, particlecorr* ppart, field &ls)
{
	...
	LOOP
	{
	lab1(i,j,k)=a->L(i,j,k);
	lab2(i,j,k)=a->L(i,j,k);
	}
	
	LOOP
	{	
	ls(i,j,k)+=p->dt*(1.0/12.0)*(23.0*a->L(i,j,k) - 16.0*lab1(i,j,k) + 5.0*lab2(i,j,k));
	
	lab2(i,j,k)=lab1(i,j,k);
	lab1(i,j,k)=a->L(i,j,k);
	}
...
void levelset_AB3::ltimesave(lexer* p, fdm *a, field &ls)
{
}
void levelset_AB3::update(lexer *p, fdm *a, ghostcell *pgc, field &f)
{
    pupdate->start(p,a,pgc);
}
