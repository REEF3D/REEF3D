...
momentum_FSFC_FS3::momentum_FSFC_FS3(lexer *p, fdm *a, ghostcell *pgc, convection *pconvection, diffusion *pdiffusion, pressure* ppressure, 
                                                    poisson* ppoisson, turbulence *pturbulence, solver *psolver, solver *ppoissonsolver, 
                                                    ioflow *pioflow, convection *ppfsfdisc, reini *ppreini, heat *&pheat, concentration *&pconc)
...
momentum_FSFC_FS3::~momentum_FSFC_FS3()
{
}

void momentum_FSFC_FS3::start(lexer *p, fdm* a, ghostcell* pgc, momentum *pmom)
{	
...
//--------------------------------------------------------
	...
	pconvec->start(p,a,ark1,4,urk1,vrk1,wrk1);
...
//--------------------------------------------------------
	...
	pconvec->start(p,a,ark2,4,urk2,vrk2,wrk2);
...
void momentum_FSFC_FS3::ucorr(lexer *p, fdm *a, double alpha, field& uvel)
{
    ULOOP
	uvel(i,j,k) -= alpha*p->dt*CPOR1*PORVAL1*((a->press(i+1,j,k)-a->press(i,j,k))
	/(p->DXP[IP]*0.5*(a->ro(i+1,j,k)+a->ro(i,j,k))));
}

void momentum_FSFC_FS3::vcorr(lexer *p, fdm *a, double alpha, field& vvel)
{
    VLOOP
	vvel(i,j,k) -= alpha*p->dt*CPOR2*PORVAL2*((a->press(i,j+1,k)-a->press(i,j,k))
	/(p->DYP[JP]*0.5*(a->ro(i,j+1,k)+a->ro(i,j,k))));
}

void momentum_FSFC_FS3::wcorr(lexer *p, fdm *a, double alpha, field& wvel)
{
    WLOOP
	wvel(i,j,k) -= alpha*p->dt*CPOR3*PORVAL3*((a->press(i,j,k+1)-a->press(i,j,k))
	/(p->DZP[KP]*0.5*(a->ro(i,j,k+1)+a->ro(i,j,k))));
}

void momentum_FSFC_FS3::irhs(lexer *p, fdm *a, ghostcell *pgc, field &f, field &uvel, field &vvel, field &wvel, double alpha)
{
...
void momentum_FSFC_FS3::jrhs(lexer *p, fdm *a, ghostcell *pgc, field &f, field &uvel, field &vvel, field &wvel, double alpha)
{
...
void momentum_FSFC_FS3::krhs(lexer *p, fdm *a, ghostcell *pgc, field &f, field &uvel, field &vvel, field &wvel, double alpha)
{
...
void momentum_FSFC_FS3::utimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_FSFC_FS3::vtimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_FSFC_FS3::wtimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_FSFC_FS3::fillaij1(lexer *p, fdm *a, ghostcell* pgc, solver *psolv)
{
}

void momentum_FSFC_FS3::fillaij2(lexer *p, fdm *a, ghostcell* pgc, solver *psolv)
{
}

void momentum_FSFC_FS3::fillaij3(lexer *p, fdm *a, ghostcell* pgc, solver *psolv)
{
}
