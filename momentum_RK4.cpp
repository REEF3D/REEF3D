...
momentum_RK4::momentum_RK4(lexer *p, fdm *a, discrete *pdiscrete, diffusion *pdiffusion, pressure* ppressure, poisson* ppoisson,
...
momentum_RK4::~momentum_RK4()
{
}

void momentum_RK4::start(lexer *p, fdm* a, ghostcell* pgc, momentum *pmom)
{
...
//--------------------------------------------------------
	...
	urk(i,j,k)  = a->u(i,j,k) + 0.5*urk1(i,j,k);
	}
	...
	vrk(i,j,k)  = a->v(i,j,k) +  0.5*vrk1(i,j,k);
	}
...
//--------------------------------------------------------
	...
	ppress->wpgrad(p,a);
	krhs(p,a,pgc,a->w,a->u,a->v,a->w,0.5);
	pdisc->start(p,a,wrk,3,urk,vrk,wrk);
...
//--------------------------------------------------------
	...
	ULOOP
	{
	urk3(i,j,k) = p->dt*CPOR1*a->F(i,j,k);
	urk(i,j,k)  = a->u(i,j,k) + urk3(i,j,k);
    }
    p->utime=pgc->timer()-starttime;
	...
	VLOOP
	{
	vrk3(i,j,k) = p->dt*CPOR2*a->G(i,j,k);
	vrk(i,j,k)  = a->v(i,j,k) +  vrk3(i,j,k);
    }
    p->vtime=pgc->timer()-starttime;
	...
	WLOOP
	{
	wrk3(i,j,k) = p->dt*CPOR3*a->H(i,j,k);
...
//--------------------------------------------------------
	...
	ULOOP
	a->u(i,j,k) = a->u(i,j,k) + (1.0/6.0)*(urk1(i,j,k) + 2.0*urk2(i,j,k) + 2.0*urk3(i,j,k) + p->dt*CPOR1*a->F(i,j,k));
    ...
    VLOOP
	a->v(i,j,k) = a->v(i,j,k) + (1.0/6.0)*(vrk1(i,j,k) + 2.0*vrk2(i,j,k) + 2.0*vrk3(i,j,k) + p->dt*CPOR2*a->G(i,j,k));
	...
	WLOOP
	a->w(i,j,k) = a->w(i,j,k) + (1.0/6.0)*(wrk1(i,j,k) + 2.0*wrk2(i,j,k) + 2.0*wrk3(i,j,k) + p->dt*CPOR3*a->H(i,j,k));
...
void momentum_RK4::irhs(lexer *p, fdm *a, ghostcell *pgc, field &f, field &uvel, field &vvel, field &wvel, double alpha)
{
...
void momentum_RK4::jrhs(lexer *p, fdm *a, ghostcell *pgc, field &f, field &uvel, field &vvel, field &wvel, double alpha)
{
...
void momentum_RK4::krhs(lexer *p, fdm *a, ghostcell *pgc, field &f, field &uvel, field &vvel, field &wvel, double alpha)
{
...
void momentum_RK4::utimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_RK4::vtimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_RK4::wtimesave(lexer *p, fdm *a, ghostcell *pgc)
{
}

void momentum_RK4::fillaij1(lexer *p, fdm *a, ghostcell* pgc, solver *psolv)
{
}

void momentum_RK4::fillaij2(lexer *p, fdm *a, ghostcell* pgc, solver *psolv)
{
}

void momentum_RK4::fillaij3(lexer *p, fdm *a, ghostcell* pgc, solver *psolv)
{
}
