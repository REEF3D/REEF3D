...
pjm_fsm::pjm_fsm(lexer* p, fdm *a) : density(p)
{
    if(p->B76==0)
    gcval_press=40;  
...
void pjm_fsm::ucorr(fdm* a, lexer* p, field& uvel,double alpha)
{	
	ULOOP
...
void pjm_fsm::vcorr(fdm* a, lexer* p, field& vvel,double alpha)
{	
	VLOOP
...
void pjm_fsm::wcorr(fdm* a, lexer* p, field& wvel,double alpha)
{	
	WLOOP
...
{
    ULOOP
    u(i,j,k)+=p->dt*alpha*PORVAL1*(a->press(i+1,j,k)-a->press(i,j,k))/(p->dx*roface(p,a,1,0,0));
    
    VLOOP
    v(i,j,k)+=p->dt*alpha*PORVAL2*(a->press(i,j+1,k)-a->press(i,j,k))/(p->dx*roface(p,a,0,1,0));
    
    WLOOP
    w(i,j,k)+=p->dt*alpha*PORVAL3*(a->press(i,j,k+1)-a->press(i,j,k))/(p->dx*roface(p,a,0,0,1));
