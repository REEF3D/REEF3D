...
reini_RK4::reini_RK4(lexer* p, fdm *a):f(p),frk1(p),frk2(p),frk3(p),L(p),dt(p)
{
	if(p->F50==1)
	gcval_phi=51;
...
void reini_RK4::start(fdm* a,lexer* p,field& b, ghostcell* pgc, ioflow* pflow)
{
    starttime=pgc->timer();
...
// **********************************************************
	...
	if(p->count==0)
	pgc->start6V(p,frk3,gcval_iniphi);
    
    if(p->count>0)
	pgc->start6V(p,frk3,gcval_phi);
...
void reini_RK4::startV(fdm* a,lexer* p,vec &f, ghostcell* pgc,ioflow* pflow)
{ 
    
}

void reini_RK4::step(lexer* p, fdm *a)
{
	reiniter=p->F44;
}

void reini_RK4::time_preproc(lexer* p)
{
    n=0;
	BASELOOP
	{
...
void reini_RK4::inisolid(lexer* p, fdm *a)
{
	pip=4;
	n=0;
	BASELOOP
...
void reini_RK4::fsfrkioV(lexer *p, fdm *a, ghostcell *pgc, vec& f)
{
