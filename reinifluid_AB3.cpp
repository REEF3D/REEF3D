...
reinifluid_AB3::reinifluid_AB3(lexer* p, fdm *a):f(p),dab1(p),dab2(p),L(p),dt(p)
{
	if(p->F50==1)
	gcval_phi=51;
...
reinifluid_AB3::~reinifluid_AB3()
{
}

void reinifluid_AB3::start(fdm* a,lexer* p,field& b, ghostcell* pgc,ioflow* pflow)
{
	starttime=pgc->timer();
...
void reinifluid_AB3::time_preproc(lexer* p)
{
	/*
	n=0;
	LOOP
