...
reinigc_RK4::reinigc_RK4(lexer* p, fdm *a):gradient(p),brk3(p),d0(p),deltax(p->dx),epsi(p->F45*p->dx)
{
	if(p->F50==1)
	gcval_phi=51;
...
reinigc_RK4::~reinigc_RK4()
{
}

void reinigc_RK4::start(fdm* a,lexer* p,field& b,ghostcell* pgc,ioflow* pflow)
{
    field4 brk1(p),brk2(p),brk3(p);
...
// **********************************************************
	...
	LOOP
	brk1(i,j,k)=b(i,j,k)+ 0.5*dt*a->L(i,j,k);
	...
	LOOP
	brk2(i,j,k)= b(i,j,k) + 0.5*dt*a->L(i,j,k);
	...
	LOOP
	brk3(i,j,k)= b(i,j,k) + dt*a->L(i,j,k);

	if(p->count==0)
	pgc->start4(p,brk3,gcval_iniphi);

	pgc->start4(p,brk3,gcval_phi);

    // Step 4
    disc(p,a,pgc,brk3);

	LOOP
	b(i,j,k)= -(1.0/3.0)*b(i,j,k) + (1.0/3.0)*brk1(i,j,k) + (2.0/3.0)*brk2(i,j,k) + (1.0/3.0)*brk3(i,j,k)
            + (1.0/6.0)*dt*a->L(i,j,k);
...
void reinigc_RK4::constraint(lexer *p, fdm* a, ghostcell *pgc, field& b)
{
	LOOP
	{
...
void reinigc_RK4::step(fdm* a, lexer* p)
{
	dt=p->F43*deltax;
