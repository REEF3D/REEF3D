...
sflow_weno_cflux::sflow_weno_cflux(lexer* p, fdm2D *b):tttw(13.0/12.0),fourth(1.0/4.0),third(1.0/3.0),
...
sflow_weno_cflux::~sflow_weno_cflux()
{
}

void sflow_weno_cflux::start(lexer* p, fdm2D* b, slice& f, int ipol, slice& uvel, slice& vvel)
{
    if(ipol==1)
    SLICELOOP1
...
double sflow_weno_cflux::aij(lexer* p,fdm2D* b,slice& f,int ipol, slice& uvel, slice& vvel)
{
...
double sflow_weno_cflux::fx(lexer *p,fdm2D *b, slice& f, int ipol, double advec)
{
    grad = 0.0;
...
double sflow_weno_cflux::fy(lexer *p,fdm2D *b, slice& f, int ipol, double advec)
{
    grad = 0.0;
...
void sflow_weno_cflux::iqmin(lexer *p,fdm2D *b, slice& f, int ipol)
{	
	q1 = f(i-2,j);
	q2 = f(i-1,j);
	q3 = f(i,j);
...
void sflow_weno_cflux::jqmin(lexer *p,fdm2D *b, slice& f, int ipol)
{   
    q1 = f(i,j-2);
	q2 = f(i,j-1);
	q3 = f(i,j);
...
void sflow_weno_cflux::iqmax(lexer *p,fdm2D *b, slice& f, int ipol)
{
    q1 = f(i+3,j);
	q2 = f(i+2,j);
	q3 = f(i+1,j);
...
void sflow_weno_cflux::jqmax(lexer *p,fdm2D *b, slice& f, int ipol)
{
	q1 = f(i,j+3);
	q2 = f(i,j+2);
	q3 = f(i,j+1);
...
void sflow_weno_cflux::is_min(fdm2D *b, slice& f)
{
...
void sflow_weno_cflux::is_max(fdm2D *b, slice& f)
{
	is1 = tttw*pow(q1 - 2.0*q2 + q3, 2.0) + fourth*pow(q1 - 4.0*q2 + 3.0*q3, 2.0);
	is2 = tttw*pow(q2 - 2.0*q3 + q4, 2.0) + fourth*pow(q2 - q4, 2.0);
	is3 = tttw*pow(q3 - 2.0*q4 + q5, 2.0) + fourth*pow(3.0*q3 - 4.0*q4 + q5, 2.0);
