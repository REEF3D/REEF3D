...
void slice3::fieldalloc(lexer* p)
{
	int gridsize = imax*jmax;
	p->Darray(V,gridsize);
}

void slice3::dealloc(lexer* p)
{
    int a,b;
...
void slice3::resize(lexer* p)
{
    if(p->gcsl_extra3*3>gcfeldsize)
    cout<<p->mpirank<<" Slice4 Resize: "<<gcfeldsize<<" "<<p->gcsl_extra3*3<<endl;
}

void slice3::fieldgcalloc(lexer* p)
{
    gcfeldsize=p->gcsl_extra3*3;
...
double & slice3::operator[](int n)
{
	return V[n];
}

double & slice3::operator()(int ii, int jj)
{			
