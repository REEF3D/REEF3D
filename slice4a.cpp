...
void slice4a::fieldalloc(lexer* p)
{
	int gridsize = imax*jmax;
	p->Darray(V,gridsize);
}

void slice4a::dealloc(lexer* p)
{
    int a,b;
...
void slice4a::resize(lexer* p)
{
    if(p->gcsl_extra4a*3>gcfeldsize)
    cout<<p->mpirank<<" Slice4a Resize: "<<gcfeldsize<<" "<<p->gcsl_extra4a*3<<endl;
}

void slice4a::fieldgcalloc(lexer* p)
{
    gcfeldsize=p->gcsl_extra4a*3;
...
double & slice4a::operator[](int n)
{
	return V[n];
}

double & slice4a::operator()(int ii, int jj)
{			
