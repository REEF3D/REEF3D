...
sloshing_force::sloshing_force(lexer *p, fdm* a, ghostcell *pgc)
{
    ...
    // open file
	if(p->P14==0)
    result.open("REEF3D-Sloshing-Force.dat");
	
	if(p->P14==1)
	result.open("./REEF3D_Force/REEF3D-Sloshing-Force.dat");
...
void sloshing_force::force(lexer *p, fdm *a, ghostcell *pgc)
{
    ...
    if(p->gcb4[n][4]==21 || p->gcb4[n][4]==22)
    {
        ...
        {
        if(i+p->origin_i==0)
        Fx_l-=p->dx*p->dx*a->press(i,j,k);
        M+=p->dx*p->dx*a->press(i,j,k)*dist_z;
        }
        
        if(p->gcb4[n][3]==4)
        {
        if(i+p->origin_i==p->gknox-1)
        Fx_r+=p->dx*p->dx*a->press(i,j,k);
        M-=p->dx*p->dx*a->press(i,j,k)*dist_z;
        }
        ...
        {
        if(+p->origin_k==0)
        Fz+=p->dx*p->dx*a->press(i,j,k);
        M+=p->dx*p->dx*a->press(i,j,k)*dist_x;
        }
