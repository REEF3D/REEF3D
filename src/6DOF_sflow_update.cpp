...
void sixdof_sflow::updateForcing_hemisphere(lexer *p, fdm2D *b, ghostcell *pgc)
{
    ...
    SLICELOOP4
    {
        ...
        if (dist < r*r)
        {
            press(i,j) = -H*press0*sqrt(1.0 - dist/(r*r))*ramp(p);
        }
        else
        {
            press(i,j) = 0.0;
...
void sixdof_sflow::updateForcing_box(lexer *p, fdm2D *b, ghostcell *pgc)
{
	...
	SLICELOOP4
    {
        ...
        {
            press(i,j) = -H*press0*(1.0 - cl*pow(xpos/Ls,4.0))*(1.0 - cb*pow(ypos/Bs,2.0))*exp(-as*pow(ypos/Bs,2.0))*ramp(p);
        }
        else
        {
            press(i,j) = 0.0;
...
void sixdof_sflow::updateForcing_ship(lexer *p, fdm2D *b, ghostcell *pgc)
{
	...
	SLICELOOP4
    {
        ...
        press(i,j) = -H*fabs(p->W22)*p->W1*draft(i,j)*ramp(p);
    }
