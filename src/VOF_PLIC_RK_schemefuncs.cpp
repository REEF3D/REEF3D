...
void VOF_PLIC::symmetric_scheme2D_markerBased
(
    fdm* a,
    lexer* p,
    ghostcell* pgc,
...
)
{
    ...
    for(int nSweep=0; nSweep<Sweepdim; nSweep++)
    {
        ...
        updatePhasemarkers(p,a,pgc,F_n);
        LOOP
        {   
            if((a->phasemarker(i,j,k)>5.9 && a->phasemarker(i,j,k)<6.1) || (a->phasemarker(i,j,k)>3.9 && a->phasemarker(i,j,k)<4.1))
            {
                reconstructPlane_alt(a,p,F_n);
                advectPlane_forCOSMIC2D_RK(a,p,sweep,-1,uvel,vvel,wvel);
            }
            else if(a->phasemarker(i,j,k)>9.9 && a->phasemarker(i,j,k)<10.1)
                advectWater_forCOSMIC2D_RK(a,p,sweep,-1,uvel,vvel,wvel);
        }
            
        if(nSweep==1 && sweep==0)
        {
            updatePhasemarkers(p,a,pgc,F_z);
        ...
        {
            updatePhasemarkers(p,a,pgc,F_x);
