						...
						if (ip >= 0 && jp >= 0 && kp >= 0 && ip < p->knox && jp < p->knoy && kp < p->knoz)
						{
                            if(ii==0 && jj==0 && kk==0)
                            {
                                ...
                                if(fabs(phitemp)<fabs(phiaux(i,j,k)))
                                    phiaux(i,j,k)=copysign(phitemp,vofstep(i,j,k)-0.5);
                            }
                            else
                            {
                                phitemp=ShortestDistanceOnBoundaryCandidate(a,p,ii,jj,kk);
                                if(fabs(phitemp)<1E05)
                                {
                                    if(fabs(phitemp)<fabs(phiaux(ip,jp,kp)))
                                        phiaux(ip,jp,kp)=phitemp;
                                }
                                else
                                {
                                    phitemp=ProjectionPointCandidate(a,p,ii,jj,kk);
                                    if(fabs(phitemp)<1E05)
                                    {
                                        if(fabs(phitemp)<fabs(phiaux(ip,jp,kp)))
                                            phiaux(ip,jp,kp)=phitemp;
                                    ...
                                    {
                                        phitemp=IntersectionPointCandidate(a,p,ii,jj,kk);
                                        if(fabs(phitemp)<1E05)
                                        {
                                            if(fabs(phitemp)<fabs(phiaux(ip,jp,kp)))
                                            phiaux(ip,jp,kp)=phitemp;
...
)
{
    ...
    dist=nx(i,j,k)*xvx+ny(i,j,k)*xvy+nz(i,j,k)*xvz-alpha(i,j,k);
    if(-dist*(vofstep(i+ii,j+jj,k+kk)-0.5)<0.0)
...
)
{
    ...
    dist=nx(i,j,k)*(p->DXN[IP]*ii)+ny(i,j,k)*(p->DYN[JP]*jj)+nz(i,j,k)*(p->DZN[KP]*kk)-alpha(i,j,k);
...
void VOF_PLIC::redistancePhiByPlane2D_Bonn
(
    fdm* a,
    lexer* p
)
{
    ...
    for(int ii=-bandWidth;ii<bandWidth+1;ii++)
    {
        for(int kk=-bandWidth;kk<bandWidth+1;kk++)
        {
                    ...
                    if (ip >= 0 &&  kp >= 0 && ip < p->knox && kp < p->knoz)
                    {
                            if(ii==0 && kk==0)
                            {
                                phitemp=alpha(i,j,k);
                            ...
                            {
                                phitemp=ShortestDistanceOnBoundaryCandidate2D(a,p,ii,kk);
                                if(fabs(phitemp)<1E05)
                                {
                                    if(fabs(phitemp)<fabs(phiaux(ip,j,kp)))
                                        phiaux(ip,j,kp)=phitemp;
                                }
                                else
                                {
                                    phitemp=ProjectionPointCandidate2D(a,p,ii,kk);
                                    if(fabs(phitemp)<1E05)
                                    {
                                        if(fabs(phitemp)<fabs(phiaux(ip,j,kp)))
                                            phiaux(ip,j,kp)=phitemp;
                                    ...
                                    {
                                        phitemp=IntersectionPointCandidate2D(a,p,ii,kk);
                                        if(fabs(phitemp)<1E05)
                                        {
                                            if(fabs(phitemp)<fabs(phiaux(ip,j,kp)))
                                            phiaux(ip,j,kp)=phitemp;
...
double VOF_PLIC::ShortestDistanceOnBoundaryCandidate2D
(
    fdm* a,
    lexer* p,
    int ii,
...
)
{
    ...
    double xvx,xvz,dist,ret;
    xvx=0.5*p->DXN[IP]*max(-1,min(1,ii));
    xvz=0.5*p->DZN[KP]*max(-1,min(1,kk));
    dist=nx(i,j,k)*xvx+nz(i,j,k)*xvz-alpha(i,j,k);
    if(-dist*(phistep(i+ii,j,k+kk)-0.5)<0.0)
        ret=copysign(sqrt(xvx*xvx+xvz*xvz),(vofstep(i+ii,j,k+kk)-0.5));
    else 
        ret=1E06;
...
double VOF_PLIC::ProjectionPointCandidate2D
(
    fdm* a,
    lexer* p,
    int ii,
...
)
{
    ...
    double xpx, xpy,xpz, dist, ret;
    dist=nx(i,j,k)*(p->DXN[IP]*ii)+nz(i,j,k)*(p->DZN[KP]*kk)-alpha(i,j,k);
    xpx=(p->DXN[IP]*ii)-dist*nx(i,j,k);
    xpz=(p->DZN[KP]*kk)-dist*nz(i,j,k);
    if(fabs(xpx)<=p->DXN[IP]*0.5+1E-04*p->DXN[IP] && fabs(xpz)<=0.5*p->DZN[KP]+1E-04*p->DZN[KP])
    {
        double dx, dz;
        dx=(p->DXN[IP]*ii)-xpx;
        dz=(p->DZN[KP]*kk)-xpz;
        ret=copysign(sqrt(dx*dx+dz*dz),(vofstep(i+ii,j,k+kk)-0.5));
    }
    else
        ret=1E06;
...
double VOF_PLIC::IntersectionPointCandidate2D
(
    fdm* a,
    lexer* p,
    int ii,
...
)// NOT ADAPTED FOR STRETCHED GRID YET !!!
{
    ...
    for(int cubeline=1; cubeline<13; cubeline++)
    {
        ...
        Dp1=n_x*p1x+n_z*p1z-r0;
        Dp2=n_x*p2x+n_z*p2z-r0;
