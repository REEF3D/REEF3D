						...
						if (ip >= 0 && jp >= 0 && kp >= 0 && ip < p->knox && jp < p->knoy && kp < p->knoz)
						{
                            if(ii==0 && jj==0 && kk==0)
                            {
                                ...
                                if(fabs(phitemp)<fabs(phiaux(i,j,k)))
                                    phiaux(i,j,k)=copysign(phitemp,vofS2(i,j,k)-0.5);
                            }
                            else
                            {
...
)
{
    ...
    if(-dist*(vofS2(i+ii,j+jj,k+kk)-0.5)<0.0)
        ret=copysign(sqrt(xvx*xvx+xvy*xvy+xvz*xvz),(vofS2(i+ii,j+jj,k+kk)-0.5));
    else 
        ret=1E06;
...
)
{
    ...
    if(fabs(xpx)<=p->DXN[IP]*0.5+1E-04*p->DXN[IP] && fabs(xpy)<=0.5*p->DYN[JP]+1E-04*p->DYN[JP] && fabs(xpz)<=0.5*p->DZN[KP]+1E-04*p->DZN[KP])
    {
        ...
        dy=(p->DYN[JP]*jj)-xpy;
        dz=(p->DZN[KP]*kk)-xpz;
        ret=copysign(sqrt(dx*dx+dy*dy+dz*dz),(vofS2(i+ii,j+jj,k+kk)-0.5));
    }
    else
        ret=1E06;
