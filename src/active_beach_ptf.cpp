...
void iowave::active_beach_ptf(lexer *p, fdm_ptf *a, ghostcell* pgc, double *Fi_, double *Uin_, slice &Fifsf, slice &eta)
{
        ...
        Uc=eta_R*sqrt(9.81/p->wd);
			...
			KLOOP 
			{
				...
				if(p->B99==3)
				Uc=eta_R*sqrt(9.81/a->depth(i,j));
				
				if(p->B99==4)
				Uc=eta_R*p->ww*(cosh(p->wk*(p->wd+z))/sinh(a->depth(i,j)*a->depth(i,j)));
               ...
               if(p->B99==5)
               {
                   ...
                   Uc =   multiplier*fac*eta_R*sqrt(9.81/a->depth(i,j));
                   }
        ...
        PCHECK
        {
        Fi_[Ip1JK] = Fi_[IJK] + Uc*fx*1.0*p->DXP[IP1];
        Fi_[Ip2JK] = Fi_[IJK] + Uc*fx*2.0*p->DXP[IP1];
        Fi_[Ip3JK] = Fi_[IJK] + Uc*fx*3.0*p->DXP[IP1];
        }
