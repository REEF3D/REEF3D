...
hypre_aij_ptf::hypre_aij_ptf(lexer* p,fdm_ptf *e,ghostcell *pgc) : xvec(p)
{	
...
hypre_aij_ptf::~hypre_aij_ptf()
{
}

void hypre_aij_ptf::start(lexer* p,fdm_ptf *e, ghostcell* pgc, field &f, vec& rhsvec, int var)
{
    make_grid(p,pgc);
    create_solvers(p,pgc);
    
    if(var<=5)
	fill_matrix_7p(p,e,pgc,f);
    
    if(var==6)
	fill_matrix_13p(p,e,pgc,f);
    
    if(var==7)
	fill_matrix_19p(p,e,pgc,f);
	...
	fillbackvec(p,e,f,xvec,var);
    
    delete_solvers(p,pgc);
    delete_grid(p,pgc);
}

void hypre_aij_ptf::startf(lexer* p, ghostcell* pgc, field &f, vec& rhs, matrix_diag &M, int var)
{
    
}

void hypre_aij_ptf::startM(lexer* p, ghostcell* pgc, double *x, double *rhs, double *M, int var)
{
}

void hypre_aij_ptf::solve(lexer* p,fdm_ptf *e, ghostcell* pgc, vec& xvec, vec& rhsvec, int var, int gcv, int &solveriter)
{
...
void hypre_aij_ptf::fillbackvec(lexer *p, fdm_ptf *e, field &f, vec &xvec, int var)
{
...
void hypre_aij_ptf::fillxvec1(lexer* p, fdm_ptf *e, field& f)
{
}
	
void hypre_aij_ptf::fillxvec2(lexer* p, fdm_ptf *e, field& f)
{
}
	
void hypre_aij_ptf::fillxvec3(lexer* p, fdm_ptf *e, field& f)
{
}
	
void hypre_aij_ptf::fillxvec4(lexer* p, fdm_ptf *e, field& f)
{
}
