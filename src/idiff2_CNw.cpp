...
void idiff2_CN::diff_w(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &u, field &v, field &w, double alpha)
{
    starttime=pgc->timer();
	...
	visctermp = 2.0*(visc_i_j_kp+ev_i_j_kp)/(p->DZN[KP1]*p->DZP[KP])
        ...
        a->rhsvec.V[count] -= (1.0-alphaCN)*visctermp*w(i,j,k);
...
void idiff2_CN::diff_w(lexer* p, fdm* a, ghostcell *pgc, solver *psolv, field &diff, field &u, field &v, field &w, double alpha)
{
    starttime=pgc->timer();
