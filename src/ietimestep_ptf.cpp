...
ietimestep_ptf::ietimestep_ptf(lexer* p):epsi(1.0e-19),c0_orig(p->N47)
{
}

ietimestep_ptf::~ietimestep_ptf()
{
}

void ietimestep_ptf::start(fdm_ptf *e, lexer *p, ghostcell *pgc, turbulence *pturb)
{
...
// maximum reynolds stress source term
	...
	cu = MIN(cu, 2.0/((sqrt(p->umax*p->umax))/p->DXN[IP]
            ...
            + sqrt((4.0*fabs(e->maxF))/p->DXN[IP])));
            
    cv = MIN(cv, 2.0/((sqrt(p->vmax*p->vmax))/p->DYN[JP]
    
            + sqrt((4.0*fabs(e->maxG))/p->DYN[JP])));
            
    cw = MIN(cw, 2.0/((sqrt(p->wmax*p->wmax))/p->DZN[KP]
    
            + sqrt((4.0*fabs(e->maxH))/p->DZN[KP])));
    }
...
void ietimestep_ptf::ini(fdm_ptf *e, lexer* p,ghostcell* pgc)
{  
    dx = p->DXM;
...
double ietimestep_ptf::min(double val1,double val2,double val3)
{
	double mini;
...
double ietimestep_ptf::min(double val1,double val2)
{
	double mini;
...
double ietimestep_ptf::max(double val1,double val2,double val3)
{
	double maxi;
...
double ietimestep_ptf::max(double val1,double val2)
{
	double maxi;
