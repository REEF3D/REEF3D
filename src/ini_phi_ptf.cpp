...
void initialise_ptf::iniphi(fdm_ptf*e, lexer* p, ghostcell* pgc)
{
    double dx=p->DXM;
    double r;
    ...
    if((p->F60>-1.0e20 || p->F56>-1.0e20) && p->F62>-1.0e-20&& p->F63>-1.0e-20 )
    {
        ...
        LOOP
        if(p->pos_x() > p->F63)
        e->phi(i,j,k)=(phidiff/xdiff)*(p->pos_x()-p->F63) + p->phimean - p->pos_z() ;
    }
	...
	LOOP
	{
		if(e->phi(i,j,k)>(p->psi))
		H=1.0;

		if(e->phi(i,j,k)<-(p->psi))
		H=0.0;

		if(fabs(e->phi(i,j,k))<=(p->psi))
		H=0.5*(1.0 + e->phi(i,j,k)/(p->psi) + (1.0/PI)*sin((PI*e->phi(i,j,k))/(p->psi)));

		e->ro(i,j,k)= 0.0; p->W1*H + p->W3*(1.0-H);
		e->visc(i,j,k)= p->W2*H + p->W4*(1.0-H);
	}
...
void initialise_ptf::iniphi_io(fdm_ptf*e, lexer* p, ghostcell* pgc)
{
    if(p->F61>-1.0e20)
    GC4LOOP
    {
        ...
        e->phi(i-1,j,k)=p->F61-p->pos_z();
        e->phi(i-2,j,k)=p->F61-p->pos_z();
        e->phi(i-3,j,k)=p->F61-p->pos_z();
        }
    p->phiin=p->F61;
    }
    ...
    GC4LOOP
    {
        ...
        e->phi(i+1,j,k)=p->F62-p->pos_z();
        e->phi(i+2,j,k)=p->F62-p->pos_z();
        e->phi(i+3,j,k)=p->F62-p->pos_z();
        }
    } 
}

void initialise_ptf::iniphi_box(lexer* p, fdm_ptf *e, ghostcell* pgc)
{
	...
	for(qn=0;qn<p->F72;++qn)
    {
        ...
        e->phi(i,j,k)=p->F72_h[qn]-p->pos_z();

	}
}

void initialise_ptf::iniphi_surfarea(lexer* p, fdm_ptf *e, ghostcell* pgc)
{
	...
	dx = (e->phi(i+1,j,k)-e->phi(i-1,j,k))/(p->DXP[IM1]+p->DXP[IP]);
	dy = (e->phi(i,j+1,k)-e->phi(i,j-1,k))/(p->DYP[JM1]+p->DYP[JP]);
	dz = (e->phi(i,j,k+1)-e->phi(i,j,k-1))/(p->DZP[KM1]+p->DZP[KP]);
	...
	dirac = (0.5/epsi)*(1.0 + cos((PI*e->phi(i,j,k))/epsi));
