...
void iowave::active_beach_ptf(lexer *p, fdm_ptf *e, ghostcell* pgc, field4 *Fi, field4 *Uin, slice &Fifsf, slice &eta)
{
        ...
        Uc=eta_R*sqrt(9.81/p->wd);
            ...
            PFLUIDCHECK
			{
				...
				if(p->B99==3)
				Uc=eta_R*sqrt(9.81/e->depth(i,j));
				
				if(p->B99==4)
				Uc=eta_R*p->ww*(cosh(p->wk*(p->wd+z))/sinh(e->depth(i,j)*e->depth(i,j)));
               ...
               if(p->B99==5)
               {
                   ...
                   Uc =   multiplier*fac*eta_R*sqrt(9.81/e->depth(i,j));
                   }
        ...
        FPCHECK
        {
        Fi[Ip1JK] = Fi[IJK] + Uc*fx*1.0*p->DXP[IP1];
        Fi[Ip2JK] = Fi[IJK] + Uc*fx*2.0*p->DXP[IP1];
        Fi[Ip3JK] = Fi[IJK] + Uc*fx*3.0*p->DXP[IP1];
        }
