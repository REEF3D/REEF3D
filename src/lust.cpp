...
void lust::start(lexer* p, fdm* a, field& b, int ipol, field& uvel, field& vvel, field& wvel)
{
    if(ipol==1)
    ULOOP
...
double lust::aij(lexer* p,fdm* a,field& f,int ipol, field& uvel, field& vvel, field& wvel, double *DX,double *DY, double *DZ)
{
    udir=vdir=wdir=0.0;
    ...
    udir=1.0;
    /*
    fu1_uw = udir*(ivel2*f(i,j,k)- ivel1*f(i-1,j,k))
