...
nhflow_depandtime_avg_vel_lineprobe::nhflow_depandtime_avg_vel_lineprobe(lexer *p, fdm_nhf *d) : probenum(p->P94)
{
    ...
    p->Darray(next_print_time,probenum);
...
nhflow_depandtime_avg_vel_lineprobe::~nhflow_depandtime_avg_vel_lineprobe()
{
    for(n=0;n<probenum;++n)
        pout[n].close();
...
void nhflow_depandtime_avg_vel_lineprobe::start(lexer *p, fdm_nhf *d, ghostcell *pgc)
{
    if(probenum<=0)
        return;
    ...
    for(n=0;n<probenum;++n)
    {
        ...
        const double effective_dt = t1 - MAX(t0,tstart);
        if(effective_dt<=0.0)
            continue;
        ...
        for(q=0;q<point_count[n];++q)
        {
            ...
            const double valid_global = pgc->globalmax(valid_local);
            ...
            if(valid_global>0.5)
            {
                ...
                u_timeint[n][q] += uavg*effective_dt;
                v_timeint[n][q] += vavg*effective_dt;
            }
        }
        ...
        for(q=0;q<point_count[n];++q)
        {
            const double uavg = u_timeint[n][q]/time_accum[n];
            const double vavg = v_timeint[n][q]/time_accum[n];
...
void nhflow_depandtime_avg_vel_lineprobe::ini_location(lexer *p, fdm_nhf *d)
{
    for(n=0;n<probenum;++n)
    {
        flag[n] = 1;
