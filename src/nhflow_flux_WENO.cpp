...
nhflow_flux_weno::nhflow_flux_weno(lexer* p, patchBC_interface *ppBC) : weno_nug_func(p), dfdx(p), dfdy(p)
{
    pBC = ppBC;
...
nhflow_flux_weno::~nhflow_flux_weno()
{
}

void nhflow_flux_weno::reconstruct_2D(lexer* p, ghostcell *pgc, fdm_nhf*, slice& f, slice &fs, slice &fn, slice &fe, slice &fw)
{
    ULOOP
    {
    // left
...
void nhflow_flux_weno::reconstruct_3D(lexer* p, ghostcell *pgc, fdm_nhf*, double *Fx, double *Fy, double *Fs, double *Fn, double *Fe, double *Fw)
{
    ULOOP
    {
    // left
...
double nhflow_flux_weno::limiter(double v1, double v2)
{
...
void nhflow_flux_weno::iqmin(lexer *p, double *F)
{	 
    q1 = F[Im2JK];
    q2 = F[Im1JK];
    q3 = F[IJK];
...
void nhflow_flux_weno::jqmin(lexer *p, double *F)
{
    q1 = F[IJm2K];
    q2 = F[IJm1K];
    q3 = F[IJK];
...
void nhflow_flux_weno::kqmin(lexer *p, double *F)
{
    q1 = F[IJKm2];
    q2 = F[IJKm1];
    q3 = F[IJK];
...
void nhflow_flux_weno::iqmax(lexer *p, double *F)
{
    q1 = F[Im1JK];
    q2 = F[IJK];
    q3 = F[Ip1JK];
...
void nhflow_flux_weno::jqmax(lexer *p, double *F)
{
    q1 = F[IJm1K];
    q2 = F[IJK];
    q3 = F[IJp1K];
...
void nhflow_flux_weno::kqmax(lexer *p, double *F)
{
	q1 = F[IJKm1];
    q2 = F[IJK];
    q3 = F[IJKp1];
...
void nhflow_flux_weno::iqmin_sl(lexer *p, slice& f)
{	
	q1 = f(i-2,j);
	q2 = f(i-1,j);
	q3 = f(i,j);
...
void nhflow_flux_weno::jqmin_sl(lexer *p, slice& f)
{
	q1 = f(i,j-2);
	q2 = f(i,j-1);
	q3 = f(i,j);
...
void nhflow_flux_weno::iqmax_sl(lexer *p, slice& f)
{
	q1 = f(i+3,j);
	q2 = f(i+2,j);
	q3 = f(i+1,j);
...
void nhflow_flux_weno::jqmax_sl(lexer *p, slice& f)
{
	q1 = f(i,j+3);
	q2 = f(i,j+2);
	q3 = f(i,j+1);
