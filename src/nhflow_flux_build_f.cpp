...
void nhflow_flux_build_f::start_U(lexer* p, fdm_nhf *d, ghostcell *pgc)
{
    ...
    d->Fs[IJK] = d->UHs[IJK]*d->Us[IJK]
            + 0.5*fabs(p->W22)*d->ETAs(i,j)*d->ETAs(i,j) + fabs(p->W22)*d->ETAs(i,j)*d->dfx(i,j);
    
    d->Fn[IJK] = d->UHn[IJK]*d->Un[IJK]
            + 0.5*fabs(p->W22)*d->ETAn(i,j)*d->ETAn(i,j) + fabs(p->W22)*d->ETAn(i,j)*d->dfx(i,j);
    }
    ...
    // flux z-dir
    WLOOP
    d->Fz[IJK] = 0.5*(d->omegaF[FIJKp1]*(d->Ub[IJK] + d->Ut[IJK]) - 0.0*fabs(d->omegaF[FIJKp1])*(d->Ut[IJK] - d->Ub[IJK]));
}
...
void nhflow_flux_build_f::start_V(lexer* p, fdm_nhf *d, ghostcell *pgc)
{
    ...
    d->Fe[IJK] = d->VHe[IJK]*d->Ve[IJK] 
            + 0.5*fabs(p->W22)*d->ETAe(i,j)*d->ETAe(i,j) + fabs(p->W22)*d->ETAe(i,j)*d->dfy(i,j);
    
    d->Fw[IJK] = d->VHw[IJK]*d->Vw[IJK] 
            + 0.5*fabs(p->W22)*d->ETAw(i,j)*d->ETAw(i,j) + fabs(p->W22)*d->ETAw(i,j)*d->dfy(i,j);
    }
    
    // flux z-dir
    WLOOP
    d->Fz[IJK] = 0.5*(d->omegaF[FIJKp1]*(d->Vb[IJK] + d->Vt[IJK]) - 0.0*fabs(d->omegaF[FIJKp1])*(d->Vt[IJK] - d->Vb[IJK]));
    }
}
...
void nhflow_flux_build_f::start_W(lexer *p, fdm_nhf *d, ghostcell *pgc)
{
    ...
    // flux z-dir
    WLOOP
    d->Fz[IJK] = 0.5*(d->omegaF[FIJKp1]*(d->Wb[IJK] + d->Wt[IJK]) - 0.0*fabs(d->omegaF[FIJKp1])*(d->Wt[IJK] - d->Wb[IJK]));
}
