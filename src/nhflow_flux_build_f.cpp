...
void nhflow_flux_build_f::start_U(lexer* p, fdm_nhf *d, ghostcell *pgc)
{
    ...
    d->Fs[IJK] = d->UHs[IJK]*d->Us[IJK]
            + 0.5*fabs(p->W22)*d->ETAs(i,j)*d->ETAs(i,j) + fabs(p->W22)*d->ETAs(i,j)*0.5*(d->depth(i-1,j)+d->depth(i,j));
    
    d->Fn[IJK] = d->UHn[IJK]*d->Un[IJK]
            + 0.5*fabs(p->W22)*d->ETAn(i,j)*d->ETAn(i,j) + fabs(p->W22)*d->ETAn(i,j)*0.5*(d->depth(i-1,j)+d->depth(i,j));
    }
...
void nhflow_flux_build_f::start_V(lexer* p, fdm_nhf *d, ghostcell *pgc)
{
    ...
    d->Fe[IJK] = d->VHe[IJK]*d->Ve[IJK] 
            + 0.5*fabs(p->W22)*d->ETAe(i,j)*d->ETAe(i,j) + fabs(p->W22)*d->ETAe(i,j)*0.5*(d->depth(i,j-1)+d->depth(i,j));
    
    d->Fw[IJK] = d->VHw[IJK]*d->Vw[IJK] 
            + 0.5*fabs(p->W22)*d->ETAw(i,j)*d->ETAw(i,j) + fabs(p->W22)*d->ETAw(i,j)*0.5*(d->depth(i,j-1)+d->depth(i,j));
    }
