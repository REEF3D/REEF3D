...
nhflow_fsf_rk_co::nhflow_fsf_rk_co(lexer *p, fdm *a, ghostcell *pgc, ioflow *pflow, patchBC_interface *ppBC) : epsi(p->A440*p->DXM)
{
    pBC = ppBC;
...
nhflow_fsf_rk_co::~nhflow_fsf_rk_co()
{
}

void nhflow_fsf_rk_co::start(lexer* p, fdm* a, ghostcell* pgc, ioflow* pflow)
{
}

void nhflow_fsf_rk_co::ltimesave(lexer* p, fdm *a, slice &ls)
{
}

void nhflow_fsf_rk_co::update(lexer *p, fdm *a, ghostcell *pgc, slice &f)
{
    pupdate->start(p,a,pgc);
}

void nhflow_fsf_rk_co::ini(lexer *p, fdm *a, ghostcell *pgc, ioflow *pflow)
{
}

void nhflow_fsf_rk_co::step1(lexer* p, fdm* a, ghostcell* pgc, ioflow* pflow, field &u, field&v, field&w, slice& etark1, slice &etark2, double alpha)
{
    SLICELOOP1
    a->P(i,j)=0.0;
...
void nhflow_fsf_rk_co::step2(lexer* p, fdm* a, ghostcell* pgc, ioflow* pflow, field &u, field&v, field&w, slice& etark1, slice &etark2, double alpha)
{
    SLICELOOP1
    a->P(i,j)=0.0;
...
void nhflow_fsf_rk_co::step3(lexer* p, fdm* a, ghostcell* pgc, ioflow* pflow, field &u, field&v, field&w, slice& etark1, slice &etark2, double alpha)
{
    // fill eta_n
    SLICELOOP4
    {
    ...
    SLICELOOP4
    a->eta(i,j) = (1.0-alpha)*a->eta(i,j) + alpha*etark2(i,j)

                - alpha*p->dt*((a->P(i,j)-a->P(i-1,j))/p->DXN[IP] + (a->Q(i,j)-a->Q(i,j-1))/p->DYN[JP]);
    ...
    p->sigma_update(p,a,pgc,a->eta,etark2,alpha);
    p->omega_update(p,a,pgc,u,v,w,a->eta,etark2,alpha);
}
