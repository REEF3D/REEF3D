...
nhflow_fsf_rk_nw::nhflow_fsf_rk_nw(lexer *p, fdm_nhf* d, ghostcell *pgc, ioflow *pflow, patchBC_interface *ppBC) : epsi(p->A440*p->DXM),P(p),Q(p),K(p)
{
    pBC = ppBC;
...
nhflow_fsf_rk_nw::~nhflow_fsf_rk_nw()
{
}

void nhflow_fsf_rk_nw::start(lexer* p, fdm_nhf* d, ghostcell* pgc, ioflow* pflow)
{
}

void nhflow_fsf_rk_nw::update(lexer *p, fdm_nhf* d, ghostcell *pgc, slice &f)
{
    //pupdate->start(p,a,pgc);
}

void nhflow_fsf_rk_nw::step1(lexer* p, fdm_nhf* d, ghostcell* pgc, ioflow* pflow, double *U, double *V, double *W, slice& etark1, slice &etark2, double alpha)
{
...
void nhflow_fsf_rk_nw::step2(lexer* p, fdm_nhf* d, ghostcell* pgc, ioflow* pflow, double *U, double *V, double *W, slice& etark1, slice &etark2, double alpha)
{
    wetdry(p,d,pgc,U,V,W,etark1);
...
void nhflow_fsf_rk_nw::step3(lexer* p, fdm_nhf* d, ghostcell* pgc, ioflow* pflow, double *U, double *V, double *W, slice& etark1, slice &etark2, double alpha)
{
    // fill eta_n
    SLICELOOP4
    {
...
void nhflow_fsf_rk_nw::flux_update(lexer* p, fdm_nhf* d, ghostcell* pgc, ioflow* pflow, double *U, double *V, double *W, slice& etark1, slice &etark2, double alpha)
{    
    pfluxfsf->face_flux_3D(p,pgc,d,etark2,U,V,d->Fx,d->Fy);
