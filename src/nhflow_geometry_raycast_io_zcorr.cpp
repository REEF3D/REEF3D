...
void nhflow_geometry::ray_cast_io_zcorr(lexer *p, fdm_nhf *d, ghostcell *pgc, int ts, int te)
{
	...
	double e0,e1,e2,area2,sgn,denom;
	double Px,Py,Rz;
	int is,ie,js,je;
    ...
    // footprint in x-y
	xs = MIN3(Ax,Bx,Cx);  xe = MAX3(Ax,Bx,Cx);
	ys = MIN3(Ay,By,Cy);  ye = MAX3(Ay,By,Cy);
    ...
    area2 = -abx*cay + aby*cax;          // (B-A) x (C-A)

    // triangle is vertical: a vertical ray cannot cross it
    if(fabs(area2) < eps_area)
    continue;

    sgn   = (area2 > 0.0 ? 1.0 : -1.0);  // normalise winding to CCW
    denom = 1.0/(sgn*area2);
	...
	je = MIN(p->posc_j(ye)+2, p->knoy);
        ...
        // 2D edge functions: e0 opposite C, e1 opposite A, e2 opposite B
