...
void nhflow_geometry::ray_cast_x(lexer *p, fdm_nhf *d, ghostcell *pgc, int ts, int te, double *LS)
{
	double ys,ye,zs,ze;
	double Px,Py,Pz;
	double Qx,Qy,Qz;
    ...
    if((IO[IJK] != IO[Im1JK]) || (IO[IJK] != IO[Ip1JK]))
    {
        ...
        if(IO[IJK] != IO[Ip1JK])
        {
		Px = p->global_xmin-10.0*DSM;
		Py = p->YP[JP]-psi;
		Pz = p->ZSP[IJK]+psi;
		
		Qx = p->global_xmax+10.0*DSM;
		Qy = p->YP[JP]+psi;
		Qz = p->ZSP[Ip1JK]-psi;
        }
        ...
        if(IO[IJK] != IO[Im1JK])
        {
		Px = p->global_xmax+10.0*DSM;
		Py = p->YP[JP]-psi;
		Pz = p->ZSP[IJK]+psi;
		
		Qx = p->global_xmin-10.0*DSM;
		Qy = p->YP[JP]+psi;
		Qz = p->ZSP[Im1JK]-psi;
        }
		...
		check = 0;
            ...
            for(i=0;i<p->knox;++i)
            LS[IJK]=MIN(fabs(Rx-p->XP[IP]),LS[IJK]);
