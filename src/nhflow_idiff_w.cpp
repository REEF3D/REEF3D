...
void nhflow_idiff::diff_w(lexer *p, fdm_nhf *d, ghostcell *pgc, ioflow *pflow, solver *psolv, double *WHdiff, double *WHin, double *UH, double *VH, double *WH, slice &WL, double alpha)
{
    ...
    LOOP
	{
        if(p->wet[IJ]==1 && p->DF[IJK]>0)
        {
            ...
            visc_JM1 += 0.5*(d->vb(i,j) + d->vb(i,j-1));
            visc_JP1 += 0.5*(d->vb(i,j) + d->vb(i,j+1));
            ...
            d->rhsvec.V[n] =  visc_IP*((UH[Ip1JKp1]-UH[Ip1JKm1]) - (UH[Im1JKp1]-UH[Im1JKm1]))/((p->DZN[KP]+p->DZN[KM1])*(p->DXP[IP]+p->DXP[IM1]))
						 ...
						 + (CPORNH*WHin[IJK])/(alpha*p->dt)
                            ...
                            + visc_IP*2.0*0.5*(p->sigy[FIJK]+p->sigy[FIJKp1])*(WH[IJp1Kp1] - WH[IJm1Kp1] - WH[IJp1Km1] + WH[IJm1Km1])
	...
	LOOP
	{
        if(p->wet[IJ]==1 && p->DF[IJK]>0)
        {
            ...
            {
            d->rhsvec.V[n] -= d->M.e[n]*WH[IJK]*p->y_dir;
            d->M.e[n] = 0.0;
            }
            
            if(p->flag4[IJp1K]<0 || p->DF[IJp1K]<0)
            {
            d->rhsvec.V[n] -= d->M.w[n]*WH[IJK]*p->y_dir;
            d->M.w[n] = 0.0;
            }
