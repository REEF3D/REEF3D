...
void nhflow_poisson_c::start(lexer* p, fdm_nhf *d, double *P)
{	
    double sigxyz2;
    ...
    LOOP
	{
        if(p->wet[IJ]==1)
        {
            ...
            d->rhsvec.V[n] +=  CPORNH*PORVALNH*(p->sigx[FIJK]+p->sigx[FIJKp1])*(P[Ip1JKp1] - P[Im1JKp1] - P[Ip1JKm1] + P[Im1JKm1])
                            /(p->W1*(p->DXN[IP]+p->DXN[IM1])*(p->DZN[KP]+p->DZN[KM1]))
                        
                            + CPORNH*PORVALNH*(p->sigy[FIJK]+p->sigy[FIJKp1])*(P[IJp1Kp1] - P[IJm1Kp1] - P[IJp1Km1] + P[IJm1Km1])
	...
	LOOP
	{
        if(p->wet[IJ]==1)
        {
            if(p->flag4[Im1JK]<0)
            {
            d->rhsvec.V[n] -= d->M.s[n]*P[Im1JK];
            d->M.s[n] = 0.0;
            }
            
            if(p->flag4[Ip1JK]<0)
            {
            d->rhsvec.V[n] -= d->M.n[n]*P[Ip1JK];
            d->M.n[n] = 0.0;
            }
            
            if(p->flag4[IJm1K]<0)
            {
            d->rhsvec.V[n] -= d->M.e[n]*P[IJm1K]*p->y_dir;
            d->M.e[n] = 0.0;
            }
            
            if(p->flag4[IJp1K]<0)
            {
            d->rhsvec.V[n] -= d->M.w[n]*P[IJp1K]*p->y_dir;
            d->M.w[n] = 0.0;
            }
            ...
            */
            ...
            if(p->flag4[IJKp1]<0)
            {
                ...
                /*d->M.t[n] -= -(sigxyz2*CPORNH*PORVALNH)/(pd->roface(p,a,0,0,1)*p->DZP[KP]*p->DZN[KP])     
                        + CPORNH*PORVALNH*0.5*(p->sigxx[FIJK]+p->sigxx[FIJKp1])/(d->ro(i,j,k)*(p->DZN[KP]+p->DZN[KM1]));
                        
                d->M.t[n] += -(sigxyz2*CPORNH*PORVALNH)/(pd->roface(p,a,0,0,1)*teta*p->DZP[KP]*p->DZN[KP])     
                        + CPORNH*PORVALNH*0.5*(p->sigxx[FIJK]+p->sigxx[FIJKp1])/(d->ro(i,j,k)*(teta*p->DZN[KP]+p->DZN[KM1]));
                */
                ...
                d->rhsvec.V[n] +=  CPORNH*PORVALNH*(p->sigx[FIJK]+p->sigx[FIJKp1])*( - P[Ip1JKm1] + P[Im1JKm1])
                        /(p->W1*(p->DXN[IP]+p->DXN[IM1])*(teta*p->DZN[KP]+p->DZN[KM1]))
                        
                        + CPORNH*PORVALNH*(p->sigy[FIJK]+p->sigy[FIJKp1])*( - P[IJp1Km1] + P[IJm1Km1])
                ...
                /*d->M.p[n] -= (sigxyz2*CPORNH*PORVALNH)/(pd->roface(p,a,0,0,1)*p->DZP[KP]*p->DZN[KP]);
                d->M.p[n] += (sigxyz2*CPORNH*PORVALNH)/(pd->roface(p,a,0,0,1)*teta*p->DZP[KP]*p->DZN[KP]);
