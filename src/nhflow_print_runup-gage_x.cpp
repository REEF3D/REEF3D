...
{	
    ...
    p->Darray(xloc,p->P133+1,maxknox);
    p->Darray(wsf,p->P133+1,maxknox);
    p->Iarray(flag,p->P133+1,maxknox);
	p->Iarray(wsfpoints,p->P133+1);
	

    p->Darray(xloc_all,p->P133+1,sumknox);
    p->Darray(wsf_all,p->P133+1,sumknox);
	p->Iarray(flag_all,p->P133+1,sumknox);
...
void nhflow_print_runup_gage_x::start(lexer *p, fdm_nhf *d, ghostcell *pgc, ioflow *pflow, slice &f)
{
    ...
    if(p->mpirank==0)
    {
		// open file
		if(p->P14==0)
		sprintf(name,"REEF3D-NHFLOW-runup-x-%06i.dat",num);


		if(p->P14==1)
		sprintf(name,"./REEF3D_NHFLOW_RUNUP/REEF3D-NHFLOW-runup-x-%68i.dat",num);
...
void nhflow_print_runup_gage_x::sort(double *a, double *b, int *c, int left, int right)
{
...
void nhflow_print_runup_gage_x::remove_multientry(lexer *p, double* b, double* c, int *d, int& num)
{
    int oldnum=num;
    double xval=-1.12e23;
