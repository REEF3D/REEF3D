...
nhflow_reinidisc_fsf::nhflow_reinidisc_fsf(lexer *p) :  ddweno_nug(p)
{
}
...
void nhflow_reinidisc_fsf::start(lexer *p, fdm *a, ghostcell *pgc, vec &b, vec &L, int ipol)
{
	if(ipol==4)
    {
        NLOOP4
...
void nhflow_reinidisc_fsf::disc(lexer *p, fdm *a, ghostcell *pgc, vec &b, vec &L, int *sizeM, int ipol, cpt &C)
{	
    /*
    L.V[n]=0.0;
