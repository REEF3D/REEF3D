                                        ...
                                        slice &Ds,slice &Dn, slice &De, slice &Dw)
{
    ...
    {
    d->Ss[IJK] = Un[IJK]*0.0 - 1.0*sqrt(9.81*Dn(i,j));
    d->Sn[IJK] = Un[IJK]*0.0 + sqrt(9.81*Dn(i,j));
    d->SSx[IJK] = d->Ss[IJK];
    }
    ...
    {
    d->Ss[IJK] = Us[IJK]*0.0 - sqrt(9.81*Ds(i,j));
    d->Sn[IJK] = Us[IJK]*0.0 + 1.0*sqrt(9.81*Ds(i,j));
    d->SSx[IJK] = d->Sn[IJK];
    }
    ...
    {
    d->Se[IJK] = Vw[IJK]*0.0 - 1.0*sqrt(9.81*Dw(i,j));
    d->Sw[IJK] = Vw[IJK]*0.0 + sqrt(9.81*Dw(i,j));
    d->SSy[IJK] = d->Se[IJK];
    }
    ...
    {
    d->Se[IJK] = Ve[IJK]*0.0 - sqrt(9.81*De(i,j));
    d->Sw[IJK] = Ve[IJK]*0.0 + 1.0*sqrt(9.81*De(i,j));
    d->SSy[IJK] = d->Sw[IJK];
    }
