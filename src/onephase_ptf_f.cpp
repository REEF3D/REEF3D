...
onephase_ptf_f::onephase_ptf_f(lexer *p, fdm_ptf *e, ghostcell *pgc) : ddweno_f_nug(p),uf(p),vf(p),wf(p),urk1(p),vrk1(p),wrk1(p),xphi(p),yphi(p),zphi(p)
{
    gcval_u=10;
	gcval_v=11;
	gcval_w=12;
...
onephase_ptf_f::~onephase_ptf_f()
{
}

void onephase_ptf_f::update(lexer *p, fdm_ptf *e, ghostcell *pgc, ioflow *pflow)
{   
    FLUIDLOOP
    {
    if(e->phi(i,j,k)<0.0)
