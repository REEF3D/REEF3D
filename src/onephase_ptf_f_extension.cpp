...
void onephase_ptf_f::uvel(lexer *p, fdm_ptf *e, ghostcell *pgc, field &u)
{
    FLUIDLOOP
    {
    nx0 = -(e->phi(i+1,j,k) - e->phi(i-1,j,k))/(p->DXP[IP]+p->DXP[IM1]);
    ny0 = -(e->phi(i,j+1,k) - e->phi(i,j-1,k))/(p->DYP[JP]+p->DYP[JM1]);
    nz0 = -(e->phi(i,j,k+1) - e->phi(i,j,k-1))/(p->DZP[KP]+p->DZP[KM1]);
    
    nx = ddwenox(e->phi,nx0);
    ny = ddwenoy(e->phi,ny0);
    nz = ddwenoz(e->phi,nz0);
...
void onephase_f::vvel(lexer *p, fdm_ptf *e, ghostcell*, field&)
{
}

void onephase_f::wvel(lexer *p, fdm_ptf *e, ghostcell *pgc, field &w)
{
    for(int qn=0;qn<2;++qn)
    {
    WFLUIDLOOP
   e->test(i,j,k) =   wrk1(i,j,k) = w(i,j,k) 
