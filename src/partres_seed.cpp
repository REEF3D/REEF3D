...
void partres::seed_ini(lexer *p, fdm *a, ghostcell* pgc)
{
    // ini
    LOOP
    {
...
/// Calls \p partres::posseed_box and \p partres::posseed_topo based on \p lexer::Q110 and \p lexer::Q101.
void partres::seed(lexer *p, fdm *a)
{
    if(p->Q110>0)
        posseed_box(p,a);
	if(p->Q101>0)
...
void partres::posseed_box(lexer *p, fdm *a)
{
    seed_srand(p);
...
void partres::posseed_box_dummy(lexer *p, fdm *a)
{
    seed_srand(p);
...
void partres::posseed_topo(lexer *p, fdm *a)
{
    seed_srand(p);
...
void partres::posseed_suspended(lexer *p, fdm *a)
{
    seed_srand(p);
...
void partres::point_source(lexer *p, fdm *a)
{
    size_t index;
...
void partres::topo_influx(lexer *p, fdm *a)
{
    seed_srand(p);
    for(int n=0;n<p->gcin_count;n++)
    {
...
void partres::seed_srand(lexer *p)
{
    if(p->Q29>0)
        srand(p->Q29);
...
void partres::seed_topo(lexer *p, fdm *a)
{
...
void partres::solid_influx(lexer *p, fdm *a)
{
    seed_srand(p);
    PLAINLOOP
...
size_t partres::set_active_topo(lexer *p, fdm *a)
{
...
void partres::seedDummy(lexer *p, fdm *a, particles_obj &PP)
{
    if(p->origin_i==0)
    {
...
void partres::seedDummyCell(lexer *p, fdm *a, particles_obj &PP)
{
    double tolerance = 5e-18;
