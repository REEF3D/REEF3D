...
pjm_sigco::pjm_sigco(lexer* p, fdm *a, ghostcell *pgc, heat *&pheat, concentration *&pconc) : teta(1.0)
{
...
void pjm_sigco::start(fdm* a,lexer*p, poisson* ppois,solver* psolv, ghostcell* pgc, ioflow *pflow, field& uvel, field& vvel, field& wvel, double alpha)
{
...
void pjm_sigco::ucorr(lexer* p, fdm* a, field& uvel,double alpha)
{	
    if(p->D37==1)
	ULOOP
...
void pjm_sigco::vcorr(lexer* p, fdm* a, field& vvel,double alpha)
{	
    if(p->D37==1)
    VLOOP
...
void pjm_sigco::wcorr(lexer* p, fdm* a, field& wvel,double alpha)
{
    if(p->D37==1)
    WLOOP 	
...
void pjm_sigco::rhs(lexer *p, fdm* a, ghostcell *pgc, field &u, field &v, field &w,double alpha)
{
    NLOOP4
	a->rhsvec.V[n]=0.0;
...
void pjm_sigco::bedbc(lexer *p, fdm* a, ghostcell *pgc, field &u, field &v, field &w,double alpha)
{
    
}
 
void pjm_sigco::vel_setup(lexer *p, fdm* a, ghostcell *pgc, field &u, field &v, field &w,double alpha)
{

}

void pjm_sigco::upgrad(lexer*p,fdm* a, slice &eta, slice &eta_n)
{
    if(p->D38==1 && p->A540==1)
    ULOOP
...
void pjm_sigco::vpgrad(lexer*p,fdm* a, slice &eta, slice &eta_n)
{
    if(p->D38==1 && p->A540==1)
    VLOOP
...
void pjm_sigco::wpgrad(lexer*p,fdm* a, slice &eta, slice &eta_n)
{
}
