...
void ptf_fsf_vtp::print(lexer* p, fdm_ptf *e, ghostcell *pgc)
{
...
//  Velocity
    ...
    for(n=0;n<vertice_num;++n)
	{
	ffn=float(p->ccipol1(e->u,ccpt[n][0],ccpt[n][1],ccpt[n][2]));
	result.write((char*)&ffn, sizeof (float));

	ffn=float(p->ccipol2(e->v,ccpt[n][0],ccpt[n][1],ccpt[n][2]));
	result.write((char*)&ffn, sizeof (float));

	ffn=float(p->ccipol3(e->w,ccpt[n][0],ccpt[n][1],ccpt[n][2]));
	result.write((char*)&ffn, sizeof (float));
	}
