...
ptf_fsfbc::ptf_fsfbc(lexer *p, fdm_ptf *e, ghostcell *pgc) : Fx(p),Fy(p),Fz(p),Ex(p),Ey(p),bx(p),by(p)
{    
    if(p->A311==0)
...
void ptf_fsfbc::fsfdisc(lexer *p, fdm_ptf *e, ghostcell *pgc, slice &eta, slice &Fifsf, field &Fi)
{
    // 3D
    if(p->j_dir==1)
    FILOOP4
...
void ptf_fsfbc::fsfwvel(lexer *p, fdm_ptf *e, ghostcell *pgc, slice &eta, slice &Fifsf)
{
    // fi
    FILOOP4
    {
    kvel = (e->Fi(i,j,k) - e->Fi(i,j,k-1))/(p->DZP[KP]);
    
    //if(p->A323<4)
    Fz(i,j) = pconvec->fz(p,e->Fi,kvel,kvel);
...
void ptf_fsfbc::kfsfbc(lexer *p, fdm_ptf *e, ghostcell *pgc)
{
    SLICELOOP4
    e->K(i,j) = - Fx(i,j)*Ex(i,j) + Fy(i,j)*Ey(i,j) 
    
                + Fz(i,j)*(1.0 + pow(Ex(i,j),2.0) + pow(Ey(i,j),2.0));
                
}

void ptf_fsfbc::dfsfbc(lexer *p, fdm_ptf *e, ghostcell *pgc, slice &eta)
{
    SLICELOOP4
    e->K(i,j) = - 0.5*pow(Fx(i,j),2.0) - 0.5*pow(Fy(i,j),2.0) 
