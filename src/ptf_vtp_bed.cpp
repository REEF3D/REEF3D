...
ptf_vtp_bed::ptf_vtp_bed(lexer *p, fdm_ptf *e, ghostcell *pgc)
{
	if(p->I40==0)
    {
	p->printtime=0.0;
...
ptf_vtp_bed::~ptf_vtp_bed()
{
}

void ptf_vtp_bed::start(lexer *p, fdm_ptf *e, ghostcell* pgc, ioflow *pflow)
{	
    print2D(p,e,pgc);
}

void ptf_vtp_bed::print2D(lexer *p, fdm_ptf *e, ghostcell* pgc)
{	
	...
	{
	// Triangle 1
	iin=int(e->nodeval2D(i-1,j-1))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval2D(i,j-1))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval2D(i,j))-1;
	result.write((char*)&iin, sizeof (int));
