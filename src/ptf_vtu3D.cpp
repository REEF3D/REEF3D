...
#include"force_fit.h"
#include"ioflow.h"
#include"ptf_print_wsf.h"
#include"ptf_print_wsf_theory.h"
#include"ptf_print_wsfline.h"
#include"ptf_print_wsfline_y.h"
#include"ptf_vtp_fsf.h"
#include"ptf_vtp_bed.h"
#include"ptf_breaking_log.h"
#include"ptf_print_Hs.h"
#include"potentialfile_out.h"
#include"ptf_state.h"
#include<sys/stat.h>
#include<sys/types.h>

ptf_vtu3D::ptf_vtu3D(lexer* p, fdm_ptf *e, ghostcell *pgc)
{	
    if(p->I40==0)
    {
	p->printtime=0.0;
    ...
    pwsf=new ptf_print_wsf(p,c);

    pwsf_theory=new ptf_print_wsf_theory(p,e,pgc);

    pwsfline=new ptf_print_wsfline(p,e,pgc);

    pwsfline_y=new ptf_print_wsfline_y(p,e,pgc);

    if(p->P230>0)
    ppotentialfile = new potentialfile_out(p,e,pgc);

    if(p->P180==1)
	pfsf = new ptf_vtp_fsf(p,e,pgc);

    pbed = new ptf_vtp_bed(p,e,pgc);

    if(p->P40>0)
	pstate=new ptf_state(p,e,pgc);

    if(p->P59==1)
    pbreaklog=new ptf_breaking_log(p,e,pgc);
	...
	for(n=0;n<p->P85;++n)
	pforce_ale[n]=new force_ale(p,e,pgc,n);
    

    for(n=0;n<p->P86;++n)
    pforce_fit[n]=new force_fit(p,e,pgc,n);

    if(p->P110==1)
    phs = new ptf_print_Hs(p,e->Hs);
...
void ptf_vtu3D::start(lexer* p, fdm_ptf* c,ghostcell* pgc, ioflow *pflow)
{
    // Gages
	if(p->P51>0)
	pwsf->height_gauge(p,e,pgc,e->eta);

    if(p->P50>0)
    pwsf_theory->height_gauge(p,e,pgc,pflow);
    
    if(p->P110==1)
    phs->start(p,pgc,e->eta,e->Hs);
    ...
    if(p->P59==1)
    pbreaklog->write(p,e,pgc);
	
	// ALE force
	  if((p->count==0 || p->count==p->count_statestart) && p->P85>0)
	  {
		for(n=0;n<p->P85;++n)
        pforce_ale[n]->ini(p,e,pgc);
	  }
        if(p->count>0 && p->P85>0)
		{
        for(n=0;n<p->P85;++n)
        pforce_ale[n]->start(p,e,pgc);
		}
    ...
    // FIT force
        ...
        {
            for(n=0;n<p->P86;++n)
            pforce_fit[n]->ini(p,e,pgc);
        }
        if(p->count>0 && p->P86>0)
        {
            for(n=0;n<p->P86;++n)
            pforce_fit[n]->start(p,e,pgc);
        }
}

void ptf_vtu3D::print_stop(lexer* p, fdm_ptf *e, ghostcell* pgc)
{
    if(p->P180==1)
    pfsf->start(p,e,pgc);
...
void ptf_vtu3D::print_vtu(lexer* p, fdm_ptf *e, ghostcell* pgc)
{
    SLICELOOP4
    {
    if(e->breaking(i,j)==1)
    e->breaking_print(i,j)=1.0;

    if(e->breaking(i,j)==0)
    e->breaking_print(i,j)=0.0;
    }

     //
    pge->start7V(p,e->Fi,e->bc,250);
    pge->gcsl_start4(p,e->WL,50);
    pge->gcsl_start4(p,e->bed,50);
    pge->gcsl_start4(p,e->breaking_print,50);
    pge->start4(p,e->test,1);

    pge->dgcslpol(p,e->WL,p->dgcsl4,p->dgcsl4_count,14);
    pge->dgcslpol(p,e->breaking_print,p->dgcsl4,p->dgcsl4_count,14);
    pge->dgcslpol(p,e->bed,p->dgcsl4,p->dgcsl4_count,14);

    e->WL.ggcpol(p);
    e->breaking_print.ggcpol(p);

    i=-1;
    j=-1;
    if(i+p->origin_i==-1 && j+p->origin_j==-1 )
    e->WL(i,j) = e->WL(i+1,j+1);
...
//  Velocities
    ...
    TPLOOP
	{
	ffn=float(e->U[FIJKp1]);

    if(k==-1 && j==-1)
	ffn=float(e->U[FIJp1Kp1]);
	result.write((char*)&ffn, sizeof (float));


	ffn=float(e->V[FIJKp1]);

    if(k==-1 && j==-1)
	ffn=float(e->V[FIJp1Kp1]);
	result.write((char*)&ffn, sizeof (float));


	ffn=float(e->W[FIJKp1]);

    if(k==-1 && j==-1)
	ffn=float(e->W[FIJp1Kp1]);
	result.write((char*)&ffn, sizeof (float));
	}
...
//  Fi
	...
	TPLOOP
	{
    ffn=float(e->Fi[FIJKp1]);

    if(k==-1 && j==-1)
	ffn=float(e->Fi[FIJp1Kp1]);
	result.write((char*)&ffn, sizeof (float));
	}
...
//  elevation
    ...
    TPLOOP
	{
	ffn=float(p->ZN[KP1]*e->WL(i,j) + e->bed(i,j));
	result.write((char*)&ffn, sizeof (float));
	}
...
//  XYZ
    ...
    TPLOOP
	{
    waterlevel = p->sl_ipol4eta(p->wet,e->eta,e->bed)+p->wd - p->sl_ipol4(e->bed);

    zcoor = p->ZN[KP1]*waterlevel + p->sl_ipol4(e->bed);
    ...
    zcoor = p->ZN[KP1]*e->WL(i,j) + e->bed(i,j);

    ffn=float( (p->XN[IP1]-p->B192_3)*eos(theta_y*sin(phase)) - (zcoor-p->B192_4)*sin(theta_y*sin(phase)) + p->B192_3);
	result.write((char*)&ffn, sizeof (float));

	ffn=float(p->YN[JP1]);
	result.write((char*)&ffn, sizeof (float));

	ffn=float((p->XN[IP1]-p->B192_3)*sin(theta_y*sin(phase)) + (zcoor-p->B192_4)*eos(theta_y*sin(phase)) + p->B192_4);
	result.write((char*)&ffn, sizeof (float));
	}
...
//  Connectivity
	...
	{
	iin=int(e->nodeval(i-1,j-1,k-1)-1);
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval(i,j-1,k-1))-1;
	result.write((char*)&iin, sizeof (int));

    iin= int(e->nodeval(i,j,k-1))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval(i-1,j,k-1))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval(i-1,j-1,k))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval(i,j-1,k))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval(i,j,k))-1;
	result.write((char*)&iin, sizeof (int));

	iin=int(e->nodeval(i-1,j,k))-1;
	result.write((char*)&iin, sizeof (int));
	}
