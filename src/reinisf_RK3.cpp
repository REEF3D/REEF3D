...
reinisf_RK3::reinisf_RK3(lexer* p, int type) : epsi(p->F45*p->DXM),f(p),frk1(p),frk2(p),L(p),dt(p)
{
	if(p->F50==1)
	gcval_phi=51;
...
reinisf_RK3::~reinisf_RK3()
{
}

void reinisf_RK3::start(fdm* a,lexer* p,field& b,ghostcell* pgc,ioflow* pflow)
{ 
...
void reinisf_RK3::startV(fdm* a,lexer* p,vec &f, ghostcell* pgc,ioflow* pflow)
{ 
    starttime=pgc->timer();
...
void reinisf_RK3::step(fdm* a, lexer* p)
{
	reiniter=p->F44;
}

void reinisf_RK3::time_preproc(lexer* p)
{
    n=0;
	FLUIDLOOP
	{
