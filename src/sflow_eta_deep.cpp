...
void sflow_eta::wetdrydeep(lexer* p, fdm2D* b, ghostcell* pgc, slice &WL)
{
    SLICELOOP4
    p->deep[IJ]=p->wet[IJ];
    ...
    if(p->A221==0)
    {
        ...
        SLICELOOP4
        if(WL(i,j)<=10.0*wd_criterion)
        p->deep[IJ]=0;
    }
    
    SLICELOOP1
    b->deep1(i,j) = (p->deep[IJ]==1 && p->deep[Ip1J]==1)?1:0;
    
    SLICELOOP2
    b->deep2(i,j) = (p->deep[IJ]==1 && p->deep[IJp1]==1)?1:0;
