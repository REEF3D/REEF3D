...
void sflow_pjm_hs::start(lexer*p, fdm2D *b, solver* psolv, ghostcell* pgc, ioflow *pflow, slice &WL, double *U, double *V, double *W, double alpha)
{
}
...
void sflow_pjm_hs::vel_setup(lexer *p, fdm2D *b, ghostcell *pgc, double *U, double *V, double *W, double alpha)
{
}

void sflow_pjm_hs::upgrad(lexer*p, fdm2D *b, slice &WL)
{
    LOOP
    WETDRY
    d->F[IJK] += 0.5*(d->ETAs(i,j)+d->ETAn(i-1,j))*fabs(p->W22)*
                (d->dfx(i,j) - d->dfx(i-1,j))/(p->DXN[IP]);
}

void sflow_pjm_hs::vpgrad(lexer*p, fdm2D *b, slice &WL)
{
    LOOP
    WETDRY
	d->G[IJK] += 0.5*(d->ETAe(i,j)+d->ETAw(i,j-1))*fabs(p->W22)*
                 (d->dfy(i,j) - d->dfy(i,j-1))/(p->DYN[JP]);
}

void sflow_pjm_hs::wpgrad(lexer*p, fdm2D *b, slice &WL)
{
}
*/