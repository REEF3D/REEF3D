...
vts3D::vts3D(lexer* p, fdm *a, ghostcell *pgc) : eta(p)
{
    if(p->F50==1)
	gcval_phi=51;
...
void vts3D::start(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, ioflow *pflow, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
	...
	if(p->count%p->P20==0 && p->P30<0.0 && p->P34<0.0 && p->P10==3 && p->P20>0)
	{
	print3D(a,p,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed);
	}

	// Print out based on time
	if((p->simtime>p->printtime && p->P30>0.0 && p->P34<0.0 && p->P10==3) || (p->count==0 &&  p->P30>0.0))
	{
	print3D(a,p,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed);
	...
	if((p->sedtime>p->sedprinttime && p->P34>0.0 && p->P30<0.0 && p->P10==3) || (p->count==0 &&  p->P34>0.0))
	{
	print3D(a,p,pgc,pturb,pheat,psolv,pdata,pconc,pmp,psed);
...
void vts3D::print3D(fdm* a,lexer* p,ghostcell* pgc, turbulence *pturb, heat *pheat, solver *psolv, data *pdata, concentration *pconc, multiphase *pmp, sediment *psed)
{
    ...
    if(p->mpirank==0)
    pvts(a,p,pgc,pturb,pheat,pdata,pconc,pmp,psed);
