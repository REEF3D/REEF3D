...
weno_flux_nug_dir::weno_flux_nug_dir(lexer* p):weno_nug_func(p)
{
    if(p->B269==0)
    {
        if(p->D11==1)
...
weno_flux_nug_dir::~weno_flux_nug_dir()
{
}

void weno_flux_nug_dir::start(lexer* p, fdm* a, field& b, int ipol, field& uvel, field& vvel, field& wvel)
{
   /* uf=vf=wf=0;
    ...
    if(p->G2==1)
    {
        ...
        uf=1;
        ULOOP
        a->F(i,j,k)+=aij_sig(p,a,b,1,uvel,vvel,wvel,p->DXP,p->DYN,p->DZN);
        }
        ...
        vf=1;
        VLOOP
        a->G(i,j,k)+=aij_sig(p,a,b,2,uvel,vvel,wvel,p->DXN,p->DYP,p->DZN);
        }
        ...
        wf=1;
        WLOOP
        a->H(i,j,k)+=aij_sig(p,a,b,3,uvel,vvel,wvel,p->DXN,p->DYN,p->DZP);
        }

        if(ipol==4)
        FLUIDLOOP
        a->L(i,j,k)+=aij_sig(p,a,b,4,uvel,vvel,wvel,p->DXN,p->DYN,p->DZN);
        
        if(ipol==5)
        LOOP
        a->L(i,j,k)+=aij_sig(p,a,b,5,uvel,vvel,wvel,p->DXN,p->DYN,p->DZN);
    }*/
}

double weno_flux_nug_dir::aij_x(lexer* p,fdm* a,field& b,int ipol, field& uvel, field& vvel, field& wvel, double *DX,double *DY, double *DZ)
{
...
double weno_flux_nug_dir::aij_y(lexer* p,fdm* a,field& b,int ipol, field& uvel, field& vvel, field& wvel, double *DX,double *DY, double *DZ)
{
		pflux->u_flux(a,ipol,uvel,ivel1,ivel2);
        pflux->v_flux(a,ipol,vvel,jvel1,jvel2);
        pflux->w_flux(a,ipol,wvel,kvel1,kvel2);
...
double weno_flux_nug_dir::aij_z(lexer* p,fdm* a,field& b,int ipol, field& uvel, field& vvel, field& wvel, double *DX,double *DY, double *DZ)
{
		pflux->u_flux(a,ipol,uvel,ivel1,ivel2);
        pflux->v_flux(a,ipol,vvel,jvel1,jvel2);
        pflux->w_flux(a,ipol,wvel,kvel1,kvel2);
...
double weno_flux_nug_dir::aij_sig(lexer* p,fdm* a,field& b,int ipol, field& uvel, field& vvel, field& wvel, double *DX,double *DY, double *DZ)
{
		pflux->u_flux(a,ipol,uvel,ivel1,ivel2);
        pflux->v_flux(a,ipol,vvel,jvel1,jvel2);
        pflux->w_flux(a,ipol,wvel,kvel1,kvel2);
...
double weno_flux_nug_dir::fx(lexer *p,fdm *a, field& b, field& uvel, int ipol, double advec)
{
    grad = 0.0;
...
double weno_flux_nug_dir::fy(lexer *p,fdm *a, field& b, field& vvel, int ipol, double advec)
{
    grad = 0.0;
...
double weno_flux_nug_dir::fz(lexer *p,fdm *a, field& b, field& wvel, int ipol, double advec)
{
    grad = 0.0;
...
void weno_flux_nug_dir::iqmin(lexer *p, field& f, field& uvel, int ipol)
{	
	q1 = f(i-2,j,k);
	q2 = f(i-1,j,k);
	q3 = f(i,j,k);
...
void weno_flux_nug_dir::jqmin(lexer *p, field& f, field& vvel, int ipol)
{
	q1 = f(i,j-2,k);
	q2 = f(i,j-1,k);
	q3 = f(i,j,k);
...
void weno_flux_nug_dir::kqmin(lexer *p, field& f, field& wvel, int ipol)
{
	q1 = f(i,j,k-2);
	q2 = f(i,j,k-1);
	q3 = f(i,j,k);
...
void weno_flux_nug_dir::iqmax(lexer *p, field& f, field& uvel, int ipol)
{
    q1 = f(i-1,j,k);
	q2 = f(i,j,k);
	q3 = f(i+1,j,k);
...
void weno_flux_nug_dir::jqmax(lexer *p, field& f, field& vvel, int ipol)
{
	q1 = f(i,j-1,k);
	q2 = f(i,j,k);
	q3 = f(i,j+1,k);
...
void weno_flux_nug_dir::kqmax(lexer *p, field& f, field& wvel, int ipol)
{
	q1 = f(i,j,k-1);
	q2 = f(i,j,k);
	q3 = f(i,j,k+1);
