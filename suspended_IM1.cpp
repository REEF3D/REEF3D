...
void suspended_IM1::start(fdm* a, lexer* p, discrete* pdisc, diffusion* pdiff, solver* psolv, ghostcell* pgc, ioflow* pflow)
{
    starttime=pgc->timer();
    clearrhs(p,a);
...
void suspended_IM1::timesource(lexer* p, fdm* a, field& fn)
{
    ...
    LOOP
    {
        ...
        a->rhsvec.V[count] += a->L(i,j,k) + concn(i,j,k)/PDT;
