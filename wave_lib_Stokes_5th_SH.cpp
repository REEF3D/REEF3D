...
{ 
    ...
    if(p->mpirank==0)
    {
    cout<<"Wave Tank: 5th-order Stokes SH waves; ";
...
double wave_lib_Stokes_5th_SH::wave_horzvel(lexer *p, double x, double y, double z)
{
    ...
    vel = c0*sqrt(fabs(p->W22)/wk)
         *((eps*a11 + pow(eps,3.0)*a31 + pow(eps,5.0)*a51)*cosh(wk*(wd+z))*cos(teta)
         + (pow(eps,2.0)*a22 + pow(eps,4.0)*a42)*cosh(2.0*wk*(wd+z))*cos(2.0*teta)
         + (pow(eps,3.0)*a33 + pow(eps,5.0)*a53)*cosh(3.0*wk*(wd+z))*cos(3.0*teta)
         + (pow(eps,4.0)*a44)*cosh(4.0*wk*(wd+z))*cos(4.0*teta)
         + (pow(eps,5.0)*a55)*cosh(5.0*wk*(wd+z))*cos(5.0*teta));
...
double wave_lib_Stokes_5th_SH::wave_w(lexer *p, double x, double y, double z)
{
    ...
    vel = c0*sqrt(fabs(p->W22)/wk)
         *((eps*a11 + pow(eps,3.0)*a31 + pow(eps,5.0)*a51)*sinh(wk*(wd+z))*sin(teta)
         + (pow(eps,2.0)*a22 + pow(eps,4.0)*a42)*sinh(2.0*wk*(wd+z))*sin(2.0*teta)
         + (pow(eps,3.0)*a33 + pow(eps,5.0)*a53)*sinh(3.0*wk*(wd+z))*sin(3.0*teta)
         + (pow(eps,4.0)*a44)*sinh(4.0*wk*(wd+z))*sin(4.0*teta)
         + (pow(eps,5.0)*a55)*sinh(5.0*wk*(wd+z))*sin(5.0*teta));
